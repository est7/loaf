...
The authoritative protocol currently declares rev 5.3.
...
- **Release flow (`chore(release): vX.Y.Z`)**: bump `package.json` version → `bun run build` (regenerate `dist/cli.mjs`) → CHANGELOG: add **both** the `## [X.Y.Z]` entry **and** the bottom `[X.Y.Z]: …/releases/tag/vX.Y.Z` reference-link line → bump README install pins → `git add -f dist/cli.mjs` + the doc/manifest files → commit → push the exact release commit to `origin/main` → run `bun run ga:check` from a clean checkout → tag that exact `origin/main` OID as `vX.Y.Z` → push the tag. The bottom link line is **not optional**: CI's `release-consistency` job (`scripts/ga-consistency-check.sh`, runs on the tag ref) fails `CHANGELOG_MISSING` on the `## [X.Y.Z]` heading alone (check 3). The same gate also enforces **tag commit == origin/main** (check 4 `HEAD_NOT_ORIGIN`). Skipping the rebuild ships a stale binary: git consumers get the old `--version` even though `package.json` bumped. Verify the working-tree binary with `node dist/cli.mjs --version` before commit and run the full GA gate after the release commit is remotely reachable. If a post-tag fix is required, stop and coordinate a new version or an explicitly authorized tag replacement; never move a published tag as a routine repair.
