...
const deliverEntry = () =>
  ({
    ...
    payload: { reason: "ship it now (verify-min)" },
  }) as never;
...
describe("verify-min — per-task required evidence (§3.2)", () => {
  test("behavioral done + local-check covering → pass", () => {
    expect(
      run(snap([task("T-001", "behavioral")], [ev("EV-1", "local-check", ["T-001"])])).ok,
    ).toBe(true);
  });
  ...
  test("structural done + local-check → pass", () => {
    expect(
      run(snap([task("T-001", "structural")], [ev("EV-1", "local-check", ["T-001"])])).ok,
    ).toBe(true);
  });
  ...
  test("visual-ui done + visual-review → pass", () => {
    expect(
      run(snap([task("T-001", "visual-ui")], [ev("EV-1", "visual-review", ["T-001"])])).ok,
    ).toBe(true);
  });

  test("visual-ui done + manual → pass", () => {
    expect(run(snap([task("T-001", "visual-ui")], [ev("EV-1", "manual", ["T-001"])])).ok).toBe(
      true,
    );
  });

  test("docs done + task-summary → pass; docs + local-check only → INCOMPLETE", () => {
    expect(run(snap([task("T-001", "docs")], [ev("EV-1", "task-summary", ["T-001"])])).ok).toBe(
      true,
    );
    expect(run(snap([task("T-001", "docs")], [ev("EV-1", "local-check", ["T-001"])])).ok).toBe(
      false,
    );
  });

  test("chore done + local-check → pass", () => {
    expect(run(snap([task("T-001", "chore")], [ev("EV-1", "local-check", ["T-001"])])).ok).toBe(
      true,
    );
  });
  ...
  test("waiver covering any kind → pass (human escape)", () => {
    expect(
      run(snap([task("T-001", "behavioral")], [ev("EV-1", "waiver", ["T-001"], "waived")])).ok,
    ).toBe(true);
  });
  ...
  ] as const)("$label → DELIVER_VERIFY_MIN_INCOMPLETE", ({ taskKind, evidenceKind, result }) => {
    const r = run(snap([task("T-001", taskKind)], [ev("EV-1", evidenceKind, ["T-001"], result)]));
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.code).toBe("DELIVER_VERIFY_MIN_INCOMPLETE");
  });
