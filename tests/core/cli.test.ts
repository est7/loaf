...
describe("End-to-end SPEC content → spec-lock approve (Slice A SC-A2)", () => {
  test("init → submit → add-req → tasks submit → advance → gate decide spec-lock --approve walks through", async () => {
    ...
    r = await cli(["tasks", "submit", "--input", tasksFile, "--format", "json"]);
    expect(r.exit).toBe(0);
    ...
    r = await cli(["status", "--format", "json"]);
    expect(r.exit).toBe(0);
...
describe("loaf finding raise --action amend-tasks — Item 3 SC1 CLI shell", () => {
  test("post-lock EXECUTE.work amend-tasks: bare FND-id stdout + journal back-edge + iteration bump + finding open", async () => {
    ...
    r = await cli(["finding", "list", "--format", "json"]);
    expect(r.exit).toBe(0);
...
// ─────────────────────────────────────────────────────────────────────────
// Phase 11 Item 3 SC2 — CLI shell test for `loaf finding raise --action
// fix-impl`. fix-impl co-emits a 3-entry batch [finding:raised,
// event:task_step_reset, event:phase_advanced(back_edge → EXECUTE.work)].
// The reset returns the target task's implement step to `pending` and
// reopens the task to `in_progress`; the finding stays open.
// ─────────────────────────────────────────────────────────────────────────

describe("loaf finding raise --action fix-impl — Item 3 SC2 CLI shell", () => {
  test("fix-impl on a done task: 3-entry batch resets implement step + reopens task + bumps iteration", async () => {
    ...
    expect(tail[0]!.payload.target).toEqual({ task_id: "T-001", step: "implement" });
    ...
    expect(tail[2]!.payload.back_edge).toEqual({ action: "fix-impl", finding_id: "FND-001" });
    ...
    const status = JSON.parse((await cli(["status", "--format", "json"])).stdout);
    expect(status.state.sub_state).toBe("EXECUTE.work");
    expect(status.state.iteration).toBe(iterBefore + 1);

    // Assertion: the target task reopened to in_progress, implement step
    // back to pending.
    const tasks = JSON.parse((await cli(["tasks", "list", "--format", "json"])).stdout).tasks;
    const t = tasks.find((x: { id: string }) => x.id === "T-001");
    expect(t.status).toBe("in_progress");
    expect(t.steps.implement.status).toBe("pending");
    // The sibling red step keeps its passed status — only implement reset.
  ...
  test("fix-impl without --target-task/--target-step fails cleanly (FINDING_TARGET_REQUIRED)", async () => {
...
describe("loaf archive — Item 2", () => {
  ...
  test("missing --reason → exit 2 (commander)", async () => {
    ...
    const r = await runCli(
      ["archive", "--feature", "auth-refresh", "--feature-dir", dir, "--format", "json"],
      { env: { LOAF_USER: "tester@example.invalid" } },
    );
    expect(r.exit).toBe(2);
  });
...
describe("loaf abandon — Item 2", () => {
  ...
  test("missing --reason → exit 2 (commander)", async () => {
    ...
    const r = await runCli(
      ["abandon", "--feature", "auth-refresh", "--feature-dir", dir, "--format", "json"],
      { env: { LOAF_USER: "tester@example.invalid" } },
    );
    expect(r.exit).toBe(2);
  });
