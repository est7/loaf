...
| `loaf hook <event>` | **Phase 16 SC-15a 框架阶段**:Claude Code hook 入口。`<event>` enum 限定 4 值(`session-start` / `write-guard` / `scope-track` / `closure-check`,详见 §11);`bare loaf hook` → exit 2 USAGE 列出 enum(`detail.events`);未知 event → exit 2 USAGE + did-you-mean;`--list-events` 显式 dump(text mode 每行 `<event>\t<ClaudeCodeName>`,JSON 模式 `{ok:true, count, events:[{event, claude_code}]}`)。**SC-15a 框架阶段**:已知 event 全部 exit 2 `HOOK_EVENT_NOT_IMPLEMENTED`(`detail.{event, sub_cycle, claude_code}`),手动调用得到明确"未实装"诊断而非静默 allow(避免 write-guard 假阳性"已生效"的安全幻觉)。**SC-15b** 实装 `session-start` + `closure-check`(read-side);**SC-15c** 实装 `write-guard` + `scope-track`(write-side);全部实装后 `HOOK_EVENT_NOT_IMPLEMENTED` 在 runtime 不再触发,catalog 留 reserved-for-future-events(参 `TASK_STATUS_WITHOUT_PROOF` 模式)| 0 / 2 |
