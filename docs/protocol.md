...
| <code>LOCK_TIMEOUT</code> | 2 | <code>could not acquire .loaf/&lt;feature&gt;/.lock within &#123;timeout_seconds&#125;s</code> | <code>another loaf process is holding the lock (see LOCK_HELD_BY for details); wait for it to release, or run `loaf doctor` to unlink the lock if its PID has exited</code> | <code>protocol.md#§11.2</code> |
...
| <code>WRITE_CONTENTION</code> | 2 | <code>another writer holds the per-feature lock at &#123;lock_path&#125;; retry after it releases</code> | <code>a concurrent `loaf` invocation is mid-write on this feature — retry once it finishes. If no writer is active, a prior run crashed mid-write: remove the stale `.lock` and run `loaf doctor` to verify journal integrity.</code> | <code>protocol.md#§11.2</code> |
...
| `--dry-run` | `-n` | bool | false | **rev 4.1**:mutating 命令只校验不落盘(见下方 dry-run 契约);read-only 命令 reject + exit 2。**v0.1.0 当前实现**:mutator 走 Pass 0/1/1.5 + Pass A 上下文完整性检查,然后短路;`MutateContext.dryRun` 控制开关;read-only 命令 reject 走 typed diagnostic `DRY_RUN_NOT_APPLICABLE`(见 ERROR_CATALOG);trace.jsonl 在 dry-run 下被抑制(§4.10);`loadSession` 跳过 mkdir 避免 feature 目录侧效。next-id 枚举 + §11.2 step 1/10 lock acquire/release + sidecar `.tmp-*` staging + doctor --rebuild dry-run 均 future。`spec edit --input` 走 mutator dry-run且不改 work copy；无 `--input` 的 editor wrapping lane 与 `tui` reject |
