...
| **Read-only 命令**(`status` / `tasks list` / `tasks next` / `tasks complete` / `tasks check` / `finding list` / `pending list` / `pending status` / `verify status` / `check <path>` / `<artifact> schema --format=json` / `evidence schema` / `resume` / `sessions list` / `tui` / `doctor`(含 `--rebuild`,v0.1.0 reject;in-memory replay precheck future)/ `--version` / `--help`)| **reject** `--dry-run`:exit 2 `DRY_RUN_NOT_APPLICABLE`(typed diagnostic;text 模式 stderr 渲染 `error: DRY_RUN_NOT_APPLICABLE — --dry-run not applicable to read-only command \`<command>\``;json 模式 ok:false 结构,`detail.command` + `detail.command_type` carry 命令标签与类别)|
