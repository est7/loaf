...
| `MISSING_INPUT` | mutator 命令未传 `--input` 或 `--input -` 时 stdin 读取失败(closed/EAGAIN 等;Phase 16 SC-4b 扩) | A3 |
...
**rev 5.x r5 catch-up runtime codes**(all are exit 2 family; source of truth for templates is `src/core/error-catalog.ts`):
...
| `DELIVER_VERIFY_MIN_UNAVAILABLE` | exit 2 | **v0.1.0 stub, SUPERSEDED at v0.1.1** — no longer emitted at runtime (reserved-for-history). quick/light `EXECUTE.done` deliver now runs the real verify-min check | — (see `DELIVER_VERIFY_MIN_INCOMPLETE`) |
| `DELIVER_VERIFY_MIN_INCOMPLETE` | exit 2 | **v0.1.1** — quick/light `loaf deliver` at `EXECUTE.done`; ≥1 done task lacks its §3.2 per-kind verify-min evidence (code→local-check / visual-ui→visual-review\|manual / docs→task-summary\|manual / chore→local-check\|manual\|task-summary; waiver always satisfies) | For each listed task add covering evidence (`loaf evidence add` — code tasks need a `local-check` run) or `loaf waive` it, then `loaf deliver` again |
