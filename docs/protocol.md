...
| `--dry-run` | `-n` | bool | false | **rev 4.1**:mutating 命令只校验不落盘(见下方 dry-run 契约);read-only 命令 reject + exit 2。**v0.1.0 当前实现**:mutator 走 Pass 0/1/1.5 + Pass A 上下文完整性检查,然后短路;`MutateContext.dryRun` 控制开关;read-only 命令 reject 走 typed diagnostic `DRY_RUN_NOT_APPLICABLE`(见 ERROR_CATALOG);trace.jsonl 在 dry-run 下被抑制(§4.10);`loadSession` 跳过 mkdir 避免 feature 目录侧效。next-id 枚举 + §11.2 step 1/10 lock acquire/release + sidecar `.tmp-*` staging + doctor --rebuild dry-run 均 future。`spec edit` / `tui` wrapping 命令未实装,占位 reject 待这两个落地后启用 |
...
| **Wrapping 命令**(`spec edit`)| reject `--dry-run`(无 mutation 意图直接落到 wrap 程序;`detail.command_type` 携带 `"wrapping"`)|
...
| `loaf spec edit [--feature <value>] [--feature-dir <value>]` | **Phase 16 SC-12a**:wrapping mutator — 起 `$EDITOR`(默认 `vi`)on `<feature-dir>/spec.md`,save 后 reload + validate + emit `event:spec_submitted` batch(复用 SC-12a-1 共享 `buildSpecSubmitBatch`)。`$EDITOR` 支持带参形态(`vim -f` / `code -w` / 引号 `"node" '<stub>.js' --flag` 等;详 §10.7 EDITOR tokenization);无 shell exec,filepath 末位 argv 拼接。No-op 跳过:editor 0 退 + 内容无变化 → exit 0 + `no_op:true` 不写 journal。失败时(editor 信号 / 非零退出 / spawn error / frontmatter 校验失败)spec.md **work copy 保留**让人改完重跑;journal 不动。frontmatter 校验失败 subcode 跟 SC-9c `check` 平行:`spec-not-found` / `missing-frontmatter` / `invalid-yaml` / `zod`。**CLI owns `spec_version` stamping**:用户改 spec_version frontmatter 字段会被忽略,emit 的 entry 永远 `current+1`(reducer monotonic 兜底)。signal 中断 → exit 130;editor 非零退出 → exit 2 `USAGE` + `detail.editor_exit`;spawn error → exit 2 `USAGE` + `detail.spawn_error`;`--dry-run` 在 spawn editor 前 reject `DRY_RUN_NOT_APPLICABLE` (`command_type: "wrapping"`)。post-lock 守门:在 spawn $EDITOR **之前**检查 `state.spec_locked === true` → exit 2 `SPEC_LOCKED_NO_DIRECT_EDIT`,引导用户走 `finding raise --action amend-spec`(避免编辑器开了用户改了再发现写不进的体验断裂)。malformed `$EDITOR`(unmatched quote 等 tokenize 错误)→ exit 2 `USAGE` + `detail.spawn_error="EDITOR_TOKENIZE_ERROR"`,journal/spec.md 都不动 | 0 / 2 |
...
v1 的核心目标是 protocol 可靠,不是知识复利自动化。跨 feature 查阅统一 grep `.loaf/*/lessons.md`;不存在 `loaf evidence list` 命令,新 lesson 也不进入 evidence projection。
