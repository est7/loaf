...
> **Authority**: Advisory(自由 markdown,不强校验;**v0.1.0 / Phase 16 SC-11**:`loaf lessons add` 仅 emit `evidence:added` 进 evidence ledger(payload.kind=`manual`,内容进 `summary` 字段,>8KB 走 LongTextField sidecar);`lessons.md` projection writer **已实装(F-024 / v0.1.1)**:每次 mutate 从 kind=manual lesson 选择器重建顶层 `.loaf/<feature>/lessons.md`,见 §12)。
...
| `loaf deliver` | **rev 3.1:advisory only,不碰 git/gh**;emit `session:delivered`(`human:` actor;reducer 直接 cursor flip 到 DONE.delivered,**不**经 `event:phase_advanced`)+ 打印 advisory `next:` 提示;spike hard block。**rev 4.1 + 5.x + Slice 1.D sub-cycle 2**:有效 source sub-state 取决于 ceremony —— `verify_phase=false`(`quick` / `light`)从 `EXECUTE.done` 调用(**v0.1.1:verify-min 已实装** —— 跑 §3.2 per-task evidence gate:code 任务需 `local-check` / visual-ui 需 `visual-review`|`manual` / docs 需 `task-summary`|`manual` / chore 需 `local-check`|`manual`|`task-summary`,`waiver` 永远满足,bug-RED 防御镜像 verify-accept;缺证据 → `DELIVER_VERIFY_MIN_INCOMPLETE` exit 2,通过 → `DONE.delivered`。注:`verify_phase=true`(standard/deep)从 `EXECUTE.done` 误 deliver → `DELIVER_NOT_ACCEPTED`(须先过 VERIFY)。light "REQ coverage not closed" 提示仍延后);`verify_phase=true && settle_phase=false`(`standard`)从 `VERIFY.accept` 调用(VERIFY 已走完,无 verify-min 二次跑;preflight step 5c 校验 `verify_accepted=true` 否则 `DELIVER_NOT_ACCEPTED`、`settle_phase=false` 否则 `DELIVER_SETTLE_PHASE_BYPASS`);`settle_phase=true`(`deep`)从 `SETTLE.lessons` 调用(reconcile.json + lesson manual evidence 已记;`lessons.md` projection writer **已实装(F-024 / v0.1.1)**,见 §4.7 Authority 备注;preflight 同样校验 `verify_accepted=true`)。任何 source 都校验 snapshot.tasks 无非-abandoned spike 任务(`DELIVER_SPIKE_TASKS`,§703 + §1298) | 0 / 2 |
| `loaf archive --reason "..."` | 不交付关闭 | 0 / 2 |
| `loaf abandon --reason "..."` | 中途放弃(reason required) | 0 / 2 |
| `loaf lessons add` | **rev 5.0 / Phase 16 SC-11**:sugar wrapper over `evidence:added` payload.kind=`manual`。形态:`loaf lessons add (--text "..." \| --file <path>) --reason "..."`。`--text` 内联 lesson body 与 `--file` 读文件互斥(exactly-one);`--reason` ≥10 字符独立于 lesson body(EvidenceFullPayload refine 要求)。actor 必须 `human:*`,`result=passed`。Lesson 内容进 `summary` 字段;byte length > `SIDECAR_THRESHOLD_BYTES`(8KB,来自 `src/core/journal-entry.ts`)时 builder 输出 `LongTextField` inline 形态,Pass 2 sidecar promote 到 `attachments/<JE-id>/`(`>` 不是 `>=`,与 sidecar predicate 对齐)。CLI 通过共享 `allocateNextEvidenceId(snapshot)` 分配 EV-id。**v0.1.1(F-024)**:`lessons.md` projection writer 已实装 —— 每次 mutate 从 lesson 选择器(kind=manual + 空 covers + 无 task_id/check/gate + human actor)重建顶层 `.loaf/<feature>/lessons.md`;`loaf lessons add` advisory 声称 `lessons.md updated` | 0 / 2 |
...
| `loaf lessons add` | **Phase 16 SC-11**:emit `evidence:added` payload.kind=`manual`(actor `human:*`,reason ≥10,lesson body 进 `summary`,>8KB 走 LongTextField sidecar)。**v0.1.1(F-024)**:`lessons.md` projection writer 已实装;CLI advisory 声称 `lessons.md updated` |
...
**rev 4.1 + 5.x + Phase 16 SC-11:quick / light / standard + lessons 说明**:quick / light / standard 三档均跳过 SETTLE.lessons sub_state(rev 5.x:standard 也砍 SETTLE),但 `loaf lessons add` 命令本身**任意 phase 可调**(append-only,不动 state machine)。user 在 non-deep session 想记教训 → 调 `loaf lessons add` 即可,**v0.1.1(F-024)**:落 evidence ledger(payload.kind=`manual`,可 `loaf evidence list --kind manual` 查)**并**由 projection writer 重建顶层 `.loaf/<feature>/lessons.md`。loaf-skill 可以 stderr **soft suggestion** "已记 lesson;考虑下个相似 feature 用 deep profile 进 SETTLE.lessons 强化复利",但**不**强制 protocol 升 profile。
...
v1 的核心目标是 protocol 可靠,不是知识复利自动化。**v0.1.0 跨 feature 模式查阅 path**:手工 grep `evidence.json | jq '.[] | select(.kind=="manual")'` 跨 features 即可 80% 解决;`.loaf/*/lessons.md` 跨 feature grep 自 F-024(v0.1.1)projection writer 落地后可用。
