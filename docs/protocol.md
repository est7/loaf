...
| <code>OPEN_FINDINGS_PRESENT</code> | 2 | <code>verify-accept check 2: &#123;count&#125; finding(s) still open (ids=&#123;open_ids&#125;); resolve or close before verify-accept</code> | <code>run `loaf finding close &lt;FND-id&gt; --resolution &lt;text&gt;` for each listed finding, OR add evidence + raise a follow-up finding if the gap is real. verify-accept check 2 requires snapshot.findings to have no entries with status=open.</code> | <code>protocol.md#§5.2</code> |
