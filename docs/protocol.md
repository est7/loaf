...
| `loaf tasks submit --input <value> [--feature <value>] [--feature-dir <value>]` | 提交完整 task graph(SPEC.design 阶段)。**rev 5.0**:走 §11.2 transaction,emit `event:tasks_planned`(payload 含完整 task array);reducer 派生 `snapshots/tasks.json`。**Phase 16 SC-4b**:`--input <src>` 接统一 modality(`-` stdin / inline JSON / file path,§10.7);whole-graph **single object** only(payload.tasks 内是 array,但 caller 不传顶层 array)。原 positional `<file>` + `-` magic 已砍。 | 0 / 2 |
| `loaf tasks add [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>] [--finding <value>]` | **rev 4.3** + **rev 5.0** + **Item 3 SC1b**:单条或 batch task 加入。`SPEC.design` 阶段(无 `--finding`)emit 整图 whole-replacement `event:tasks_planned`;`EXECUTE.work` 阶段(带 `--finding <FND-N>`)是 sponsored 路径 —— 每个新增 task emit 一条 `event:tasks_amended` `mode=add` + `sponsored_by_finding_id`(input 含多个 task 时走 `mutateBatch`)。`--finding` 在 `SPEC.design` → `USAGE` 拒绝;无 `--finding` 在非 `SPEC.design` → `SUB_STATE_AUTHORITY_VIOLATION`。原 `<T-N>` positional + `--kind` / `--drives` per-field flag **已砍**;CLI 在 lock 内单调分配 `T-id`(`^T-\d{3,}$`)并 stdout 回打;reducer 派生 `snapshots/tasks.json`。**Phase 16 SC-4b**:`--input <src>` 接统一 modality(`-` stdin / inline JSON / file path,§10.7);**batch-capable**(在 `INPUT_SCHEMAS`,接 single object 或非空数组)。原 positional `<file>` + `-` magic 已砍。 | 0 / 2 |
