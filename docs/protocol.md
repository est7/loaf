...
> - `lessons.md` 永久双读:单一入口 `selectLessonEntries()` 同时接新 kind 与 legacy lesson heuristic,保持 journal 顺序。新 lesson 明确不进 evidence projection / coverage / status / board / TUI / resume-pack;legacy lesson 的旧 evidence 可见性永久保留。
> - 旧 session 的 `loaf_version_required` 仍只由 `session:started` 写入,不回填。旧 CLI 读到新 kind 时:doctor `--rebuild` → `INVALID_ENTRY` exit 2;普通 replay/mutator → `UNEXPECTED_ERROR` exit 1 + crash log且禁止写;只读 projection 路径(status/TUI/board)继续工作。旧 CLI 的 `doctor --check-tail` 在 version-aware 防截断门落地前不得用于含新 kind 的 journal;该门与 H2 tail-recovery roadmap 合并,不属于本 rev runtime 改动。
>
...
> - **`SCHEMA_VERSION` 1 → 2**:envelope shape 级常量 bump;配合 per-entry `entry_schema_version`(envelope 字段)+ `UPCASTER_REGISTRY`(keyed by (kind, entry_schema_version))做 per-kind upcast。机器表达见 `src/core/journal-entry.ts` + `src/core/migration.ts`。
...
> - **`--plain` / `--format` 互斥契约**(SC-5b 激活):同值无冲突(`--format=json` 单独 OK),不同值 exit 2 `MUTUALLY_EXCLUSIVE_FLAGS`。§10.7 加归一化段 + §10.5 错误表加行 + `src/cli/flag-exclusions.ts` 新 `FLAG_EXCLUSIONS` 常量
...
> - 旧机器契约中的 `Profile` enum(`"quick" | "standard" | "deep"`)整个砍。
...
>   - §10.7 加 **`--dry-run` / `-n` global flag**(v1.0;fan-out worker pre-check 必备)+ §10.7 dry-run 契约 + `src/core/concurrency-contract.ts` `dry_run_transaction_order`
...
>   - `src/core/journal-entry.ts` 新加 `PendingId` + `PendingPromptKind`,`src/core/projection-schema.ts` 加 queue entry + state/registry pending 投影
...
> - **C9'**:`RegistryFile` 加 `feature: string`(canonical source `SessionStartedPayload.feature`,`z.string().min(1)` — **Phase 16 SC-7** confirmed kebab-case is convention for production users, NOT a schema-enforced regex,见 §14.2)。RegistryFile 在 `~/.loaf/registry/<session_id>.json`,path **不**带 feature 上下文,TUI 启动单文件读需要这字段;StateJson **不** carry 同字段(其 path 已 carry feature dir,reader 可一行 derive — 避免 SSOT 冗余)
...
> - **§17 命名修正**:文档残留把当前协议称"v3"、把历史 Python 实现称"v2",与当时的机器契约 + §15-16 + `SCHEMA_VERSION=1` 直接矛盾。统一为「当前 = loaf-cli v1」+「历史 = legacy Python 原型」,去掉 "v2" 简写,避免未来真出 v2 时撞名
>
...
| # | 原则 | 落实 |
|---|---|---|
| 1 | Protocol over implementation | 本文档定义 prose contract;[`machine-contract.md`](machine-contract.md) 索引 runtime binding |
| 2 | Single source of truth = runtime Zod | 每个 domain owner 定义一次,JSON Schema auto-derive;`loaf <artifact> schema --format=json` 自描述;**禁止手维护 .schema.json** |
| 3 | Schema IS the contract | runtime owner 里的 enum / 形状 / refine 就是契约;skills 必须产出符合 schema 的 artifact;loaf-cli 严格 reject 非合规输入 |
...
<!-- generated:fsm-substates BEGIN -->
| Sub-state | Entry condition | Exit condition | Logical write paths | Gate |
|---|---|---|---|---|
| <code>TRIAGE.score</code> | <code>loaf start &lt;desc&gt; invoked</code> | <code>complexity_score computed (0-100)</code> | <code>.loaf/&lt;feature&gt;/state.json</code> | — |
| <code>TRIAGE.confirm</code> | <code>score computed</code> | <code>user accepts or overrides profile</code> | <code>.loaf/&lt;feature&gt;/state.json</code> | — |
| <code>SPEC.proposal</code> | <code>ceremony.spec_phase=true &amp;&amp; TRIAGE.confirm done; OR Q9 escalation backfill (ceremony.spec_phase 由 false 改 true)</code> | <code>spec.md body has Proposal section</code> | <code>.loaf/&lt;feature&gt;/spec.md</code><br><code>.loaf/&lt;feature&gt;/spec-draft-context.md</code> | — |
| <code>SPEC.spec</code> | <code>proposal section exists OR amend-spec back-edge</code> | <code>frontmatter has requirements (each with three-way verifiability) + scenarios (+visual_contracts if UI); needs_clarification empty</code> | <code>.loaf/&lt;feature&gt;/spec.md</code> | — |
| <code>SPEC.plan</code> | <code>spec section complete &amp;&amp; needs_clarification empty</code> | <code>spec.md body has Plan section</code> | <code>.loaf/&lt;feature&gt;/spec.md</code> | — |
| <code>SPEC.design</code> | <code>plan section complete</code> | <code>design section + tasks.json generated; every REQ/SCEN/VIS bound to ≥1 task</code> | <code>.loaf/&lt;feature&gt;/spec.md</code><br><code>.loaf/&lt;feature&gt;/tasks.json</code> | <code>spec-lock</code> |
| <code>EXECUTE.plan</code> | <code>spec-lock passed (or quick: TRIAGE.confirm done)</code> | <code>every task has execution policy populated per its kind</code> | <code>.loaf/&lt;feature&gt;/tasks.json</code> | — |
| <code>EXECUTE.work</code> | <code>EXECUTE.plan done OR fix-impl/fix-test/amend-tasks back-edge</code> | <code>every task.status = done OR abandoned, with all required steps passed/waived/na</code> | <code>.loaf/&lt;feature&gt;/tasks.json</code><br><code>.loaf/&lt;feature&gt;/evidence.jsonl</code><br><code>.loaf/&lt;feature&gt;/findings.jsonl</code> | — |
| <code>EXECUTE.done</code> | <code>all tasks status ∈ &#123;done, abandoned&#125;</code> | <code>advance to VERIFY.plan (verify_phase=true); OR DONE.delivered (verify_phase=false: quick / light non-spike via `loaf deliver`: verify-min runs at this boundary, on pass transition direct to DONE.delivered, on fail exit 2 — see protocol.md §3.2 + §10.14)</code> | — | — |
| <code>VERIFY.plan</code> | <code>EXECUTE.done &amp;&amp; ceremony.verify_phase=true</code> | <code>applicability computed for each VerifyCheckKind (must/optional/na with reasons)</code> | <code>.loaf/&lt;feature&gt;/state.json</code> | — |
| <code>VERIFY.run</code> | <code>VERIFY.plan done with run applicability ∈ &#123;must, optional-elected&#125;; OR amend back-edge</code> | <code>run check passed or explicitly waived</code> | <code>.loaf/&lt;feature&gt;/evidence.jsonl</code><br><code>.loaf/&lt;feature&gt;/findings.jsonl</code> | — |
...
<!-- generated:fsm-substates END -->

### Transition edges(canonical MACHINE projection)

<!-- generated:fsm-transitions BEGIN -->
#### Legal `event:phase_advanced` transitions

| From | To | Owner kind | Guards |
|---|---|---|---|
| <code>TRIAGE.score</code> | <code>TRIAGE.confirm</code> | <code>event:phase_advanced</code> | — |
| <code>TRIAGE.confirm</code> | <code>SPEC.proposal</code> | <code>event:phase_advanced</code> | <code>spec_phase_required</code> |
| <code>TRIAGE.confirm</code> | <code>EXECUTE.plan</code> | <code>event:phase_advanced</code> | <code>spec_phase_forbidden</code> |
| <code>SPEC.proposal</code> | <code>SPEC.spec</code> | <code>event:phase_advanced</code> | — |
| <code>SPEC.spec</code> | <code>SPEC.plan</code> | <code>event:phase_advanced</code> | — |
| <code>SPEC.plan</code> | <code>SPEC.design</code> | <code>event:phase_advanced</code> | — |
| <code>SPEC.design</code> | <code>EXECUTE.plan</code> | <code>event:phase_advanced</code> | <code>spec_locked_required</code> |
| <code>EXECUTE.plan</code> | <code>EXECUTE.work</code> | <code>event:phase_advanced</code> | — |
| <code>EXECUTE.work</code> | <code>EXECUTE.done</code> | <code>event:phase_advanced</code> | — |
| <code>EXECUTE.done</code> | <code>VERIFY.plan</code> | <code>event:phase_advanced</code> | <code>verify_phase_required</code> |
| <code>VERIFY.plan</code> | <code>VERIFY.run</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.run</code> | <code>VERIFY.review</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.run</code> | <code>VERIFY.acceptance</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.run</code> | <code>VERIFY.visual</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.run</code> | <code>VERIFY.accept</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.review</code> | <code>VERIFY.acceptance</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.review</code> | <code>VERIFY.visual</code> | <code>event:phase_advanced</code> | — |
| <code>VERIFY.review</code> | <code>VERIFY.accept</code> | <code>event:phase_advanced</code> | — |
...
| From | To | Owner kind | Guards |
|---|---|---|---|
| <code>EXECUTE.done</code> | <code>DONE.delivered</code> | <code>session:delivered</code> | — |
| <code>VERIFY.accept</code> | <code>DONE.delivered</code> | <code>session:delivered</code> | — |
| <code>SETTLE.lessons</code> | <code>DONE.delivered</code> | <code>session:delivered</code> | — |
| <code>SETTLE.lessons</code> | <code>DONE.archived</code> | <code>session:archived</code> | — |
| <code>SETTLE.lessons</code> | <code>DONE.abandoned</code> | <code>session:abandoned</code> | — |

#### Navigation hints (not legal transitions)

| From | To | Owner kind | Guards |
|---|---|---|---|
| <code>EXECUTE.work</code> | <code>EXECUTE.work</code> | <code>contract:next</code> | — |
| <code>VERIFY.plan</code> | <code>VERIFY.review</code> | <code>contract:next</code> | — |
| <code>VERIFY.plan</code> | <code>VERIFY.acceptance</code> | <code>contract:next</code> | — |
| <code>VERIFY.plan</code> | <code>VERIFY.visual</code> | <code>contract:next</code> | — |
| <code>VERIFY.plan</code> | <code>VERIFY.accept</code> | <code>contract:next</code> | — |
| <code>VERIFY.review</code> | <code>VERIFY.run</code> | <code>contract:next</code> | — |
| <code>VERIFY.acceptance</code> | <code>VERIFY.run</code> | <code>contract:next</code> | — |
| <code>VERIFY.acceptance</code> | <code>VERIFY.review</code> | <code>contract:next</code> | — |
| <code>VERIFY.visual</code> | <code>VERIFY.run</code> | <code>contract:next</code> | — |
| <code>VERIFY.visual</code> | <code>VERIFY.review</code> | <code>contract:next</code> | — |
| <code>VERIFY.visual</code> | <code>VERIFY.acceptance</code> | <code>contract:next</code> | — |
<!-- generated:fsm-transitions END -->
...
- `tasks.json` 中无 `status="in_progress"` 的 task(active-set 部分,cross-file invariant 由 `transitions.ts` 在 `loaf advance` 时强制(`loaf tasks check` 为 inventory:future,未在 v0.1.0 实装,届时一并强制)—— Zod 单文件 refine 无法表达跨文件约束,这是 rev 4.0 fresh design 对 §1 原则 3「Schema IS the contract」的一档让步)
...
`state.ceremony` 6 字段决定本 session 的 ceremony 配置(`src/core/journal-entry.ts::Ceremony`):
...
### verify-min(防偷渡,rev 4.2;**v0.1.1 已实装**)
...
完整 Zod schema 位于 [`machine-contract.md`](machine-contract.md) 索引的 runtime owners。下面只示例 + 关键约束。
...
专用 lock 为 `~/.loaf/runtime/<session_id>.lock`(0600)，payload 为 strict `{pid, acquired_at, operation, owner}`，其中 `owner` 是每次 acquisition 唯一的 128-bit hex token。获取使用 bounded retry；PID 仍存活时永不抢占，仅在 PID 已不存在且二次读取确认 lock generation 未改变时删除 stale lock。exclusive create 后必须重读并确认自己的 token 才可进入 critical section；release 也只删除仍携带自己 token 的 lock，foreign token 保留且不报错。malformed/incomplete lock fail closed。不得复用 feature journal `.lock`：feature lease 虽采用同样的 PID/token fencing，但覆盖 journal → projection 的长事务且默认等待 30s；runtime lock 覆盖高频 hook accumulation 且等待窗口更短。需要两者的 EXECUTE closure 固定先 runtime、后 feature，代码中不存在反向获取边。
...
> **Authority**: 派生投影(reducer 从 `event:spec_req_added` / `event:spec_scenario_added` / `event:spec_visual_added` / `event:spec_submitted` entries 重建)。`SPEC.*` pre-lock 阶段允许 `loaf spec edit --input` 或 `$EDITOR` 更新工作副本,但提交必须经 CLI → journal entry(§11.2);post-lock 直写由 diff-guard 拦截(§11.1)。
...
post-lock 行为与 pre-lock 直接编辑入口的 post-lock 拒规则一致(§5.3 反向 transition):**唯一**合法回退路径是 finding。
...
机器表达见 `src/cli/input-schemas.ts` + `src/core/spec-schema.ts`(`SpecReqInput.id_namespace` / `SpecScenarioInput.id_namespace` / `SpecVisualInput.id_namespace`)。input regex 与 output regex 是**两个独立 Zod 类型**,CLI 任何阶段不可直接互转(防 LLM 在 input 里塞带序号的伪完整 id)。
...
- `tasks.execution.<step>.status` 是 **cache**,不是真理源;真理源是 `evidence.jsonl`。一致性校验由 `loaf tasks check` 跑(**inventory:future,未在 v0.1.0 实装**)

**Semantic authoring + ID 分配**(rev 5.2,A08;supersedes ADR-0004 A5 的 task-input 细节):`loaf tasks submit --input` 与 `loaf tasks add --input` 接受 strict semantic task definitions。每条 task 必须提供 invocation-local `local_key`(`^[A-Za-z][A-Za-z0-9_-]*$`,max 64);不得携 `id` / `status` / `execution` / journal envelope。`depends_on` 每项必须显式区分已存在 task (`{"task_id":"T-NNN"}`)与同批 task (`{"local_key":"name"}`);CLI 先为整批分配 ID、再解析 dependency refs,因此 forward local ref 合法。task id(`^T-\d{3,}$`)在 feature write lease 内从 journal 历史最大值后单调分配,whole-replacement 不复用已消失 ID。`execution` 由 CLI 初始化为 `status="pending"`;caller 只可用 per-kind `step_policy` 指定初始 applicability。成功 JSON 回传 `task_ids_by_local_key`。CLI 最终仍写既有 full `event:tasks_planned` / `event:tasks_amended` payload,所以 journal wire 与历史 replay 不变。旧 `{based_on,tasks:[{id,status,execution,...}]}` CLI input 被有意拒绝为 `SCHEMA_VALIDATION_FAILED`(`migration=legacy-full-input-rejected`);这是仅 authoring surface 的 approved breaking change。精确 schema:`loaf tasks submit --schema --format=json` / `loaf tasks add --schema --format=json`;runtime source 见 `src/core/task-schema.ts::TaskAuthoringInput`。
...
- Actor ownership is intentionally split: payload.actor = evidence attester;
  journal envelope actor = writer provenance. They are deliberately allowed to differ
  because the CLI writer may record evidence attested by a human, skill, or CI job.
...
`loaf evidence add --input <src>` 的 `attachments[]` 当前**要求完整 Attachment 形态** `[{ path, sha256, mime, bytes? }]`(Phase 16 SC-4c:auto-hash materialization 尚未接入)。下方简化输入形态 `[{ path }]` + transformation 7 步是 **ADR-0004 A6 / 未来 SC 的目标契约**,**当前 SC-4c 不生效**;`src/core/evidence-schema.ts` + `src/cli/input-schemas.ts::INPUT_SCHEMAS["evidence:add"]` 都仍要求 caller 提供完整 metadata(详 §10.7)。
...
理由:LLM 在 shell 调 `sha256sum` 或推断 mime 几乎必错,这是经典 **shape transformation**(path → canonical entry with hash/mime/bytes),归 CLI 完美。这一 transformation 是 ADR-0004 A6 auto-hash transaction 的核心,但 v0.1.0 / Phase 16 SC-4c 仅完成 source modality + batch 接入,**materialization 由后续 SC 落地**;在那之前 callers 仍需自己提供 `sha256`(`shasum -a 256 path | cut -d' ' -f1`)+ `mime`(`file --mime-type -b path`)+ 可选 `bytes`(`stat -f%z path` / `stat -c%s path`)。机器表达见 `src/cli/input-schemas.ts::INPUT_SCHEMAS["evidence:add"]`(SC-4c 已 patched 为 full Attachment[] + `.strict()`)。
...
> **Authority**: Advisory(自由 markdown,不强校验;**rev 5.1**:`loaf lessons add` emit strict `lesson:recorded@1`(`LSN-NNN`,内容进 `summary`,>8KB 走 LongTextField sidecar);`lessons.md` projection writer 在单一 selector 永久双读新 kind + legacy lesson-shaped `evidence:added`,见 §12)。
...
> **Authority**: Config(project-level,非 per-feature journal 一部分;初始化经 `loaf config init`,后续 mutation 经手动编辑或 `loaf config set` ——后者亦走 §11.2 transaction,但不在 `.loaf/<feature>/journal.jsonl` 体系内)。
...
2. `findings.jsonl` 中不存在 `status === open` 且 action **未声明 deferral** 的 finding；`defer` / `backlog` 的 action 本身就是处置决定，保持 open 以保留 carry-forward 语义但不阻塞 gate
...
- **spec-lock 前**:直接编辑入口(`loaf spec submit` / `loaf tasks submit` / `loaf tasks amend`)即可,无需 finding(原独立 `loaf amend --target spec|tasks` 命令已退役,能力并入这些入口 —— 见 §10.8 历史注)
...
每条 evidence 满足某个 covered ID 需要通过 `canSatisfy(evidence, coveredId)` 检查。逻辑位于 `src/core/evidence-compat.ts` 的 `EVIDENCE_COMPAT` 表:
...
兼容表(摘自 `src/core/evidence-compat.ts`):
...
`loaf evidence add` 在成功写入后，用同一个 `canSatisfy()` +
`EVIDENCE_COMPAT` owner 检查每个 `covers[]` pair。若不兼容，stderr 输出
`warning`，列出 obligation id、supplied kind 与 allowed kinds；journal entry
仍保留，exit 仍为 0，JSON stdout shape 不增加 warning 字段。检查使用完整
predicate（含 manual/waiver actor+reason、VIS visual-review attachment）；正常
成功写入已先通过 `EvidenceFullPayload`，因此这些 refine 与 kind-membership
结果一致。batch/multi-obligation 只为实际不兼容的 pair 各输出一条 warning。
...
- 其他 sub-state:不允许 raise(pre-lock 直接编辑入口即可,见 §5.3)
...
- `VerifyCheckKind`(`src/core/evidence-schema.ts`)= **check 分类数据**(`run` / `review` / `acceptance` / `visual`),用在 checklist / verify 状态投影里;扩展只是 enum 加项(未来 `security` / `performance` / `accessibility`),**不影响 protocol surface**——新增 kind 走既有 lane,evidence 走对应的 `EvidenceKind`(例如 security check 沿 `VERIFY.review` lane 走 + `EvidenceKind="verify-review"`)。
...
- `EvidenceKind`(`src/core/evidence-schema.ts`,例如 `local-check` / `verify-review` / `spec-review` / `acceptance` / `visual-review` / `manual` / `waiver` …)= **第三个轴**,独立于上两者,描述每条 evidence 的来源类别。
...
**rev 5.0 / Phase 16 SC-9a-1 + issue #20**:read-only 命令,exit 0 即"诊断完成",`all_pass` 字段单独承载"是否全过"的语义;失败信息按 `VerifyCheckId` 五行汇总,每行 `failures: FailedCheck[]` 复数(单 check 可多 failure,如多条 lane 缺失 / 多个 REQ 未覆盖)。顶层 `lanes[]` 另以固定 run/review/acceptance/visual 顺序枚举每条泳道的 `applicability: must|optional|na` 与 `reason`(非 na 时固定 `null`);check failure 仍由 `checks[].failures[].detail.lane` 承载。当前协议没有 optional 触发规则，所以运行时只会产 must/na，但保留 closed enum 以匹配 §7.3。

NA 规则按 `VerifyCheckId` 决定:`lane_status` ∅ → na;`open_findings` 始终 applicable，但只把非 `defer` / `backlog` 的 actionable open finding 算作 failure；`coverage` 0 obligation → na;`task_evidence` 在 `tasks_based_on` 已 plan 但 0 done task 时 → na(unplanned 还会跑 precondition,emit `TASKS_NOT_PLANNED`);`spec_review` 在 `strict_spec_review !== true` 时 → na。
...
光看 sub_state 名字无法区分 `SPEC.plan`(risks/milestones 阶段)与 `EXECUTE.plan`(per-task execution policy 阶段)两个"plan"的写权限差异。本表是协议层显式声明,机器表达见 `src/core/sub-state-contracts.ts::SubStateContract.mutation_rights`。
...
<!-- generated:fsm-mutation-rights BEGIN -->
...
**强制方式**:
- 字段维度:`loaf <cmd>` 在 §11.2 step 3 preflight 内做 sub_state × kind × mutation_rights refine(对应 `src/core/machine.ts` 派生的 `SUB_STATE_CONTRACTS.mutation_rights` + ADR-0005 §3.6 per-kind invariants);违反在 preflight 抛错,不进 journal
...
`tasks step done --result` 表示 **step outcome**;同一 transaction 内可选
`--evidence-result` 表示独立的 **evidence outcome**。因此 RED reproduction
已正确完成时 step 可以是 `passed`,而证据仍以 `failed` 记录被测系统的预期失败。
`tasks register-red` 只适用于已存在失败 RED test 的 behavioral bug task:
它记录 implement 之前的 **ordering proof**,不替代 RED 工作,也不是 non-bug
task 的通用完成捷径。
...
- 长输出(`loaf <artifact> schema --format=json` / `loaf sessions list` 在 100+ session 时)通过 `$PAGER`(默认 `less -FIRX`,仅 stdout 是 TTY)
...
| Env var | Tier | 用途 | 是否 loaf 专属 |
|---|---|---|---|
| `LOAF_LANG` | runtime/public | i18n 语言(`en` / `zh`,见 §18) | ✅ |
| `LOAF_NO_COLOR` | runtime/public | 禁用颜色(等价 `--no-color`) | ✅ |
| `LOAF_DEBUG` | runtime/public | 等价 `--debug`(写 trace.jsonl) | ✅ |
| `LOAF_SESSION` | runtime/public | per-shell-session sticky session_id(UUID);CLI 默认 dispatch key,被 `--session` 覆盖 | ✅ |
| `LOAF_FEATURE` | runtime/public | per-shell-session sticky feature 名;`LOAF_SESSION` 缺失时 fallback,被 `--feature` 覆盖 | ✅ |
| `LOAF_USER` | runtime/public | human actor 首选来源；未设置时仅 interactive context 可 fallback 到 git `user.email` | ✅ |
| `LOAF_REGISTRY_DIR` | runtime/public | 覆盖用户 registry root；默认 `~/.loaf/registry`，测试也用它隔离 registry | ✅ |
| `LOAF_FORMAT` <!-- inventory:future reason="env-layer follow-up after SC-5b" --> | runtime/public | `json` / `text` 二选一；当前 runtime 尚未读取 | ✅ |
| `NO_COLOR` | runtime/public | 通用；非空即禁色 | 通用 |
| `DEBUG` | runtime/public | 通用；非空等价 `LOAF_DEBUG=1` | 通用 |
| `EDITOR` | runtime/public | `loaf spec edit` / `loaf lessons add` 调用 | 通用 |
| `TERM` | runtime/public | `TERM=dumb` 禁用颜色 | 通用 |
| `LANG` | runtime/public | `LOAF_LANG` 未设置时的 locale fallback | 通用 |
| `LC_ALL` | runtime/public | locale fallback，优先于 `LC_MESSAGES` / `LANG` | 通用 |
| `LC_MESSAGES` | runtime/public | locale fallback，优先于 `LANG` | 通用 |
| `USER` | runtime/public | non-human `cli:loaf@<user>` actor suffix | 通用 |
...
- **Wrapping 命令**(`loaf spec edit` 无 `--input` 时起 `$EDITOR` / `loaf tui` 起 fullscreen TUI)期间 **Ctrl-C 由内层接管**:editor 的 `:q` / `Ctrl-X` / TUI 的 `q` 退出;loaf 本身不 trap signal 在 wrap 期。Editor / TUI 退出后控制权交回 loaf,后续 SIGINT 走 §10.4 规则
...
| **Flag mutual exclusion**(rev 4.2,clig.dev §6)| `MUTUALLY_EXCLUSIVE_FLAGS`:format flag 冲突 prose 见 §10.7(SC-5b 引入 `--plain` 后激活)+ 未来其它互斥对。exit 2,stderr 列冲突 flag 对 + `--format` 归一化提示。详见 §10.7 「Format flag 归一化与互斥」段 |
...
|---|---:|---|---|---|
| <code>INPUT_FILE_NOT_FOUND</code> | 2 | <code>input file does not exist: &#123;path&#125;</code> | <code>verify the path, or pass '-' to read from stdin / inline JSON starting with a JSON object or array — see `loaf &lt;cmd&gt; --help` for examples</code> | <code>protocol.md#§10.7</code> |
| <code>MISSING_INPUT</code> | 2 | <code>required input source missing or unreadable: --input not provided OR stdin could not be read (--input - failed)</code> | <code>pass --input with one of: a JSON file path, '-' for stdin (with valid piped JSON), or inline JSON; for stdin failures, pass valid JSON to `loaf &lt;cmd&gt; --input -` on stdin; for the 6 schema-capable authoring commands (spec add-req / spec add-scenario / spec add-visual / tasks submit / tasks add / evidence add), run `loaf &lt;cmd&gt; --schema --format=json` to view the input schema</code> | <code>protocol.md#§10.7</code> |
| <code>SPEC_EDIT_INPUT_REQUIRED</code> | 2 | <code>non-interactive `loaf spec edit` requires --input &lt;src&gt;; the editor lane requires TTY stdin and stdout</code> | <code>pass --input with a JSON object &#123;"body":"&lt;Markdown&gt;"&#125; via file, stdin '-', or inline JSON; alternatively rerun from a terminal with both stdin and stdout attached to a TTY</code> | <code>protocol.md#§10.7</code> |
| <code>SCHEMA_VALIDATION_FAILED</code> | 2 | <code>input does not satisfy schema for &#123;command&#125;: &#123;zod_path&#125;: &#123;zod_message&#125;</code> | <code>for the 6 schema-capable authoring commands (spec add-req / spec add-scenario / spec add-visual / tasks submit / tasks add / evidence add), run `loaf &#123;command&#125; --schema --format=json` to dump the input JSON Schema; for artifact projection files, run `loaf &lt;kind&gt; schema --format=json` (kind ∈ spec / tasks / evidence / finding / state). Fix the offending field and retry</code> | <code>protocol.md#§10.5</code> |
| <code>SPEC_LOCKED_NO_DIRECT_EDIT</code> | 2 | <code>&#123;kind&#125; blocked: spec_locked=true; use `loaf finding raise --category spec-gap --action amend-spec` to back-edge into SPEC.spec</code> | <code>raise a finding with category=spec-gap (or spec-defect) and action=amend-spec to back-edge into SPEC.spec (the finding's resets_spec_locked effect lifts the gate); then retry the spec add/submit</code> | <code>protocol.md#§5.3</code> |
| <code>SPEC_NOT_INITIALIZED</code> | 2 | <code>&#123;kind&#125; blocked: spec_version=0; run `loaf spec submit` first to bump spec_version to 1</code> | <code>run `loaf spec submit --input &lt;file&gt;` first to bump spec_version to 1, then retry the add-* command (SC4 will add `loaf spec init` as a separate scaffold helper that chains into submit)</code> | <code>protocol.md#§4.2</code> |
| <code>SPEC_ALREADY_INITIALIZED</code> | 2 | <code>spec.md already exists at &#123;spec_md_path&#125;; refusing to overwrite</code> | <code>edit the existing spec.md directly, or remove it before re-running `loaf spec init` (no --force flag in Slice 4)</code> | <code>protocol.md#§4.2</code> |
| <code>CONFIG_ALREADY_INITIALIZED</code> | 2 | <code>loaf config already exists at &#123;config_path&#125;; refusing to overwrite</code> | <code>edit the existing config file directly, or remove it before re-running `loaf config init` (no --force flag)</code> | <code>protocol.md#§10.8</code> |
| <code>ATTACHMENT_NOT_FOUND</code> | 2 | <code>attachment path does not exist: &#123;path&#125;</code> | <code>verify the path is reachable from the working directory and readable by the current user</code> | <code>protocol.md#§4.4</code> |
| <code>ATTACHMENT_NOT_FILE</code> | 2 | <code>attachment path is not a regular file: &#123;path&#125; (&#123;kind&#125;)</code> | <code>attachments must be regular files; directories, symlinks to directories, sockets, and FIFOs are rejected</code> | <code>protocol.md#§4.4</code> |
| <code>FINDING_ACTION_UNUSUAL_REASON_REQUIRED</code> | 2 | <code>finding category=&#123;category&#125; × action=&#123;action&#125; is 'unusual'; --reason of at least &#123;min_reason_length&#125; characters is required</code> | <code>rerun with --reason explaining why this non-typical combination applies (see references/finding-matrix-rationale.md)</code> | <code>protocol.md#§4.5</code> |
| <code>FINDING_ACTION_INCOHERENT</code> | 2 | <code>finding category=&#123;category&#125; × action=&#123;action&#125; is incoherent: no target task exists to apply this transition to</code> | <code>amend the spec first (category=spec-gap / new-scope × action=amend-spec) so a target task can be planned, then raise the fix-impl / fix-test finding against that task</code> | <code>protocol.md#§4.5</code> |
| <code>FINDING_TARGET_REQUIRED</code> | 2 | <code>finding action=&#123;action&#125; target validation failed (&#123;reason&#125;): task_id=&#123;task_id&#125;, step=&#123;step&#125;</code> | <code>fix-impl/fix-test require --target-task + --target-step matching the action's canonical step (fix-impl=implement, fix-test=red); amend-tasks accepts an optional but valid target; amend-spec / defer / backlog must not carry a target</code> | <code>protocol.md#§4.5</code> |
| <code>PRUNE_RESTORE_NOT_FOUND</code> | 2 | <code>no trashed session matches the given id</code> | <code>run `loaf prune --history` to list trashed sessions (slice 6b)</code> | <code>protocol.md#§10.8</code> |
| <code>PRUNE_RESTORE_AMBIGUOUS</code> | 2 | <code>the session id was trashed more than once; pass --at &lt;ts&gt; to pick one</code> | <code>re-run `loaf prune restore &lt;id&gt; --at &lt;ts&gt;` with one of the listed timestamps</code> | <code>protocol.md#§10.8</code> |
| <code>PRUNE_RESTORE_INCOMPLETE</code> | 2 | <code>the trash bucket is incomplete (missing a required artifact); not restoring</code> | <code>inspect the trash bucket; a complete bucket has manifest.json + registry.json</code> | <code>protocol.md#§10.8</code> |
| <code>PRUNE_PATH_OCCUPIED</code> | 2 | <code>a restore destination already exists; refusing to overwrite</code> | <code>move or remove the occupying registry entry / feature dir, then retry restore</code> | <code>protocol.md#§10.8</code> |
| <code>PRUNE_PARTIAL_FAILURE</code> | 2 | <code>prune partially failed: one or more sessions could not be removed</code> | <code>inspect detail.failed; rerun prune for the failed sessions after resolving the error</code> | <code>protocol.md#§10.8</code> |
| <code>MUTUALLY_EXCLUSIVE_FLAGS</code> | 2 | <code>mutually exclusive flags in the same invocation: &#123;flags&#125;</code> | <code>pass at most one of the flags from each exclusion set; see `loaf &lt;cmd&gt; --help` for the canonical flag list</code> | <code>protocol.md#§10.7</code> |
| <code>INVALID_ENV_VALUE</code> | 2 | <code>environment variable &#123;env_name&#125;=&#123;value&#125; is not in the accepted enum: &#123;accepted&#125;</code> | <code>unset &#123;env_name&#125; or set it to one of: &#123;accepted&#125;</code> | <code>protocol.md#§10.3</code> |
| <code>INVALID_FORMAT</code> | 2 | <code>invalid --format value '&#123;value&#125;'; allowed: &#123;allowed_values_human&#125;</code> | <code>pass --format text or --format json (the only allowed values for this release); --format=&lt;value&gt; equals form is accepted</code> | <code>protocol.md#§10.7</code> |
| <code>INVALID_LOCALE</code> | 2 | <code>invalid locale from &#123;source&#125;: &#123;value&#125; (expected &#123;accepted&#125;)</code> | <code>unset the locale override or set it to one of: &#123;accepted&#125;; user preferences live in ~/.loaf/config.json locale.default_lang</code> | <code>docs/adr/0006-runtime-i18n-and-user-config.md</code> |
| <code>DRY_RUN_NOT_APPLICABLE</code> | 2 | <code>--dry-run not applicable to &#123;command_type&#125; command `&#123;command&#125;`</code> | <code>--dry-run only applies to mutating commands; re-run without --dry-run (or -n) to invoke the &#123;command_type&#125; command</code> | <code>protocol.md#§10.7</code> |
| <code>HOOK_EVENT_NOT_IMPLEMENTED</code> | 2 | <code>hook event `&#123;event&#125;` is not implemented in this loaf version (Phase 16 SC-15&#123;sub_cycle&#125; pending; see protocol §11)</code> | <code>upgrade to a loaf release that implements this hook event, OR skip this hook surface for now — `loaf hook --list-events` shows the canonical 4-event enum</code> | <code>protocol.md#§11</code> |
| <code>TASK_STATUS_WITHOUT_PROOF</code> | 2 | <code>task &#123;task_id&#125; status change requires evidence: status=&#123;status&#125; has no PASSING covering evidence proof in evidence.jsonl</code> | <code>emit `loaf evidence add` covering task_id=&#123;task_id&#125; before advancing status (task-evidence is otherwise enforced later at verify-min / verify-accept)</code> | <code>protocol.md#§4.4</code> |
| <code>MISSING_VERIFIABILITY</code> | 2 | <code>REQ &#123;req_id&#125; must declare measurable, verified_by_scenarios[], or acceptance_na+reason</code> | <code>add one of: measurable with metric, threshold, and optional unit/direction; verified_by_scenarios: [SCEN-...]; or acceptance_na: true with acceptance_na_reason of at least 10 characters</code> | <code>protocol.md#§4.2</code> |
| <code>VAGUE_NO_SCENARIO</code> | 2 | <code>requirement &#123;req_id&#125; reads as vague but is not anchored to a measurable threshold or to a verifying scenario</code> | <code>either add measurable with a numeric threshold and direction, or add the verifying SCEN-id to verified_by_scenarios</code> | <code>protocol.md#§4.2</code> |
| <code>DRIVES_NOT_BOUND</code> | 2 | <code>REQ &#123;req_id&#125; is not referenced by any task.drives[]</code> | <code>add a task whose drives[] contains &#123;req_id&#125; (loaf tasks add --input ...), or remove the REQ if it is intentionally out-of-scope for this feature</code> | <code>protocol.md#§4.3</code> |
| <code>MUTATION_OUT_OF_RIGHTS</code> | 2 | <code>event:tasks_amended on task &#123;task_id&#125; is not permitted at sub_state &#123;sub_state&#125; — §8.6 grants no mutation right for this change</code> | <code>the mutation rights matrix (protocol.md §8.6) limits EXECUTE.plan `tasks amend` to execution[].applicability changes plus a status pending→ready advance; graph/kind-flag fields are frozen. To restructure the task graph, raise a `finding raise --action amend-tasks` back-edge, then run the sponsored `tasks add --finding` / `tasks amend --input --finding` at EXECUTE.work — a sponsored amend may change graph/definition fields but never erases execution progress (task/step status is frozen)</code> | <code>protocol.md#§8.6</code> |
| <code>LOCK_TIMEOUT</code> | 2 | <code>could not acquire .loaf/&lt;feature&gt;/.lock within &#123;timeout_seconds&#125;s</code> | <code>another loaf process is holding the feature lease; wait for it to release. A later writer automatically reclaims a lease only when its PID is verifiably dead and the owner generation is unchanged; malformed leases fail closed and require inspection.</code> | <code>protocol.md#§11.2</code> |
| <code>LOCK_INVALID</code> | 2 | <code>feature write lease at &#123;lock_path&#125; is malformed or incomplete</code> | <code>inspect the lease and active loaf processes; malformed leases fail closed and no loaf command deletes them. Remove or replace the file only after independently proving that no writer owns it.</code> | <code>protocol.md#§11.2</code> |
| <code>FEATURE_NOT_FOUND</code> | 2 | <code>no feature found in cwd (.loaf/ is empty or missing, or no projection has phase != DONE)</code> | <code>run `loaf start &lt;description&gt;` to create a new feature, or cd into a directory that already has a .loaf/&lt;feature&gt;/ subtree</code> | <code>protocol.md#§10.3</code> |
| <code>FEATURE_AMBIGUOUS</code> | 2 | <code>current working directory has &#123;count&#125; active features and no dispatch context: &#123;feature_list&#125;</code> | <code>disambiguate with --feature &lt;name&gt;, --session &lt;UUID&gt;, or set $LOAF_FEATURE / $LOAF_SESSION in the environment</code> | <code>protocol.md#§10.3</code> |
| <code>SESSION_CWD_MISMATCH</code> | 2 | <code>--session &#123;uuid&#125; is registered against cwd=&#123;registered_cwd&#125;, but the current cwd is &#123;current_cwd&#125;</code> | <code>cd to the registered cwd before issuing the command, or pass a different --session, or drop --session to auto-pick a session in the current cwd</code> | <code>protocol.md#§10.3</code> |
| <code>SESSION_SHORT_AMBIGUOUS</code> | 2 | <code>--session &#123;prefix&#125; matches &#123;match_count&#125; sessions in the registry: &#123;candidate_list&#125;</code> | <code>pass a longer UUID prefix (≥8 chars are required; use more to disambiguate) or pass the full UUID</code> | <code>protocol.md#§10.3</code> |
| <code>SESSION_NOT_FOUND</code> | 2 | <code>--session &#123;uuid_or_prefix&#125; matches no entry in the registry</code> | <code>run `loaf sessions list --in-cwd` to see registered sessions (future SC-9b), or run `loaf start &lt;name&gt;` to create one</code> | <code>protocol.md#§10.3</code> |
| <code>PENDING_BLOCKS_ADVANCE</code> | 2 | <code>pending head &#123;pending_id&#125; (kind=&#123;kind&#125;) blocks `loaf advance` until resolved</code> | <code>resolve the head with the kind-appropriate command: `loaf gate decide &lt;G&gt;` for kind=gate_decision; `loaf profile escalate --confirm --input &lt;ceremony.json&gt;` for kind=profile_escalation; `loaf pending resolve --answer &lt;a&gt;` for the rest</code> | <code>protocol.md#§10.7</code> |
| <code>GATE_NOT_PENDING</code> | 2 | <code>`loaf gate decide &#123;gate_kind&#125;` requires pending head kind=gate_decision; current head kind: &#123;head_kind&#125;</code> | <code>resolve the current head first via the kind-appropriate command, or wait for the gate_decision pending to appear</code> | <code>protocol.md#§10.7</code> |
| <code>ESCALATION_NOT_PENDING</code> | 2 | <code>`loaf profile escalate --confirm --input &lt;ceremony.json&gt;` requires pending head kind=profile_escalation; current head: &#123;actual_head&#125;</code> | <code>resolve the current head first via the kind-appropriate command, or wait for the profile_escalation pending to appear</code> | <code>protocol.md#§10.7</code> |
| <code>ACTOR_AUTHORITY_VIOLATION</code> | 2 | <code>actor &#123;actor&#125; is not allowed for journal kind &#123;kind&#125;</code> | <code>use the command surface that owns this kind; human-only kinds require an interactive human actor resolved by LOAF_USER or git user.email</code> | <code>protocol.md#§10.8</code> |
| <code>FROM_CURSOR_MISMATCH</code> | 2 | <code>entry payload.from=&#123;payload_from&#125; does not match current sub_state=&#123;current_sub_state&#125;</code> | <code>refresh the current session state and emit the transition from the actual cursor; do not replay a stale transition candidate</code> | <code>protocol.md#§11.2</code> |
...
| <code>VERIFY_PHASE_FORK_VIOLATION</code> | 2 | <code>transition &#123;from&#125; → &#123;to&#125; violates ceremony.verify_phase=&#123;verify_phase&#125;</code> | <code>follow the ceremony fork: verify_phase=true enters VERIFY.plan, verify_phase=false can deliver after minimal verification</code> | <code>protocol.md#§5.2</code> |
| <code>EXECUTE_DONE_TASKS_NOT_FINAL</code> | 2 | <code>cannot advance EXECUTE.work → EXECUTE.done: &#123;count&#125; task(s) are not in a final status (done or abandoned); finish their remaining steps or abandon out-of-scope tasks with `loaf tasks abandon &lt;T-N&gt; --reason "..."`</code> | <code>finish the remaining steps — run each task's steps via `loaf tasks step` until it auto-promotes to status=done — OR abandon out-of-scope tasks with `loaf tasks abandon &lt;T-N&gt; --reason "..."`, then retry `loaf advance EXECUTE.done`; see detail.non_final for the tasks still pending or in progress</code> | <code>protocol.md#§10.5</code> |
| <code>ALREADY_STARTED</code> | 2 | <code>session bootstrap kind &#123;kind&#125; cannot run after state already exists</code> | <code>resume the existing session or create a new feature directory instead of starting/migrating over initialized state</code> | <code>protocol.md#§11.2</code> |
| <code>FINDING_NOT_FOUND</code> | 2 | <code>finding close references unknown finding id &#123;id&#125;</code> | <code>list open findings and close an existing id, or raise the finding before closing it</code> | <code>protocol.md#§10.8</code> |
| <code>NO_SESSION</code> | 2 | <code>no session at &#123;feature_dir&#125; — run `loaf start &lt;feature&gt;` first</code> | <code>run `loaf start` or `loaf doctor --migrate-v2` before emitting non-bootstrap journal entries</code> | <code>protocol.md#§10.8</code> |
| <code>PENDING_NOT_FOUND</code> | 2 | <code>pending resolve failed: &#123;reason&#125;</code> | <code>resolve the current pending head only; list pending items and retry with the head id</code> | <code>protocol.md#§10.7</code> |
| <code>REDUCER_NOT_IMPLEMENTED</code> | 2 | <code>reducer has no handler for journal kind &#123;kind&#125;</code> | <code>do not append this kind until REDUCER_IMPLEMENTED_KINDS and reducer.apply both support it</code> | <code>protocol.md#§11.2</code> |
| <code>ENTRY_OVERSIZE</code> | 2 | <code>journal entry serialized to &#123;bytes&#125; bytes; limit is &#123;limit&#125;</code> | <code>move long text into sidecar form via LongTextField instead of embedding it inline</code> | <code>protocol.md#§11.2</code> |
| <code>SHORT_WRITE</code> | 2 | <code>journal append wrote &#123;wrote&#125; of &#123;want&#125; bytes</code> | <code>stop writing, preserve the journal, and run `loaf doctor --check-tail` before retrying</code> | <code>protocol.md#§11.2</code> |
| <code>TAIL_CORRUPTION</code> | 2 | <code>journal tail is corrupt: &#123;reason&#125;</code> | <code>run `loaf doctor --check-tail`; do not append until the tail has been repaired or quarantined</code> | <code>protocol.md#§10.15</code> |
| <code>MIGRATION_BACKUP_MISSING</code> | 2 | <code>migration backup target is unavailable: &#123;backup_dir&#125;</code> | <code>move or remove the existing backup target, then rerun `loaf doctor --migrate-v2`</code> | <code>protocol.md#§10.15</code> |
| <code>MIGRATION_INCOMPLETE</code> | 2 | <code>migration cannot complete: &#123;reason&#125;</code> | <code>fix the legacy v0.0.x artifact or restore from backup; rerun migration only after validation passes</code> | <code>protocol.md#§10.15</code> |
| <code>MIGRATION_REPLAY_ATTEMPT</code> | 2 | <code>journal.jsonl already has entries; migration must run on a fresh journal</code> | <code>do not rerun migration over an initialized journal; inspect the existing journal or start from the original v0.0.x backup</code> | <code>protocol.md#§10.15</code> |
| <code>MIGRATION_SIDECAR_MISSING</code> | 2 | <code>migration sidecar is missing: &#123;artifact&#125;</code> | <code>restore the missing legacy artifact or sidecar, then rerun migration/doctor verification</code> | <code>protocol.md#§10.15</code> |
| <code>INVALID_ACTOR_FORMAT</code> | 2 | <code>human actor value is invalid: &#123;reason&#125;</code> | <code>set LOAF_USER to the raw human identifier without a namespace prefix, or unset it to allow interactive git user.email fallback</code> | <code>protocol.md#§10.8</code> |
| <code>NO_HUMAN_ACTOR</code> | 2 | <code>no human actor could be resolved for a human-only command</code> | <code>run interactively with git user.email configured, or set LOAF_USER explicitly</code> | <code>protocol.md#§10.8</code> |
| <code>DUPLICATE_REQ_ID</code> | 2 | <code>REQ id &#123;id&#125; is already in the spec projection</code> | <code>allocate a fresh REQ id under the same id_namespace (the CLI scans for max serial + 1 inside the per-session lock) or `loaf finding raise --category spec-gap --action amend-spec` if you need to retire the existing REQ</code> | <code>protocol.md#§4.2</code> |
| <code>DUPLICATE_SCEN_ID</code> | 2 | <code>SCEN id &#123;id&#125; is already in the spec projection</code> | <code>allocate a fresh SCEN id under the same id_namespace, or amend via finding mechanism if retiring an existing scenario</code> | <code>protocol.md#§4.2</code> |
| <code>DUPLICATE_VIS_ID</code> | 2 | <code>VIS id &#123;id&#125; is already in the spec projection</code> | <code>allocate a fresh VIS id under the same id_namespace, or amend via finding mechanism if retiring an existing visual contract</code> | <code>protocol.md#§4.2</code> |
| <code>SPEC_FRONTMATTER_INVALID</code> | 2 | <code>spec.md frontmatter failed gate check 1 (subcode=&#123;subcode&#125;)</code> | <code>subcode=SPEC_NOT_FOUND: run `loaf spec init` then `loaf spec submit` to seed spec.md; subcode=SPEC_YAML_INVALID: check the `---`-fenced YAML block at the top of spec.md for syntax errors; subcode=SPEC_FRONTMATTER_INVALID: run `loaf spec schema --format=json` to dump the SpecFrontmatter JSON Schema (Phase 16 SC-10) and fix the offending field. Both spec-lock and verify-accept require a valid spec.md at check 1.</code> | <code>protocol.md#§5.1</code> |
| <code>SPEC_HAS_UNCLARIFIED</code> | 2 | <code>spec has &#123;count&#125; unresolved needs_clarification entries (ids=&#123;ids&#125;); resolve or remove them before spec-lock can pass</code> | <code>edit spec.md to remove resolved needs_clarification entries, or run `loaf finding raise --category spec-gap --action clarify` to formalize the resolution flow; spec-lock check 2 requires needs_clarification === []</code> | <code>protocol.md#§5.1</code> |
| <code>TASK_NOT_FOUND</code> | 2 | <code>task &#123;task_id&#125; is not in the current tasks projection</code> | <code>run `loaf tasks list` to see live ids; if you meant to add a new task, use `loaf tasks add` instead of amend/step; if you expected the id to exist, the projection may be stale — run `loaf doctor --rebuild` to rebuild from journal</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_STEP_NOT_FOUND</code> | 2 | <code>step &#123;step&#125; is not seeded on task &#123;task_id&#125; — seeded steps are derived from the task's kind execution schema (§14)</code> | <code>use only the per-kind step names — behavioral: red/implement/refactor; structural: implement/refactor; visual-ui: mockup/implement/screenshot-compare; docs: draft/review; spike: explore/prototype/record; chore: execute. Running an unseeded step name was a silent add bug in v0.0.x — sub-cycle 3a fails fast instead</code> | <code>protocol.md#§10.8</code> |
| <code>DUPLICATE_TASK_ID</code> | 2 | <code>task id &#123;task_id&#125; appears more than once in tasks_planned payload</code> | <code>tasks_planned is whole-replacement — each task id must be unique within the batch. Rename one or merge them in the planning input</code> | <code>protocol.md#§10.8</code> |
| <code>TASKS_NOT_PLANNED</code> | 2 | <code>gate task-graph check: tasks have not been planned (snapshot.tasks_based_on is null)</code> | <code>run `loaf tasks submit --input &lt;plan-file&gt;` to emit event:tasks_planned and seed the task graph; spec-lock check 3 and verify-accept check 4 both require tasks_based_on.spec to match the current spec.spec_version</code> | <code>protocol.md#§5.1</code> |
| <code>TASKS_BASED_ON_STALE</code> | 2 | <code>gate task-graph check: tasks_based_on.spec=&#123;tasks_based_on_spec&#125; but current spec.spec_version=&#123;current_spec_version&#125; — the task graph was planned against an older spec</code> | <code>either re-plan tasks against the current spec via `loaf tasks submit` (whole-replacement), or amend individual tasks via `loaf tasks add/amend` + raise a `loaf finding raise --category spec-gap --action amend-spec` if a spec roll-back is needed. Surfaces for spec-lock (check 3) and verify-accept (check 4 precondition).</code> | <code>protocol.md#§5.1</code> |
| <code>REQ_NOT_DRIVEN</code> | 2 | <code>spec-lock check 4: requirement &#123;req_id&#125; is not referenced by any task.drives[]</code> | <code>add a task whose drives[] array includes &#123;req_id&#125;, or remove the requirement from spec.md if it is no longer in scope. Note: this is the REQ-side coverage code (distinct from legacy DRIVES_NOT_BOUND which named the inverse direction)</code> | <code>protocol.md#§5.1</code> |
| <code>E2E_SCENARIO_UNBOUND</code> | 2 | <code>spec-lock check 6: e2e scenario &#123;scenario_id&#125; has no binding task (requires task with requires_acceptance=true AND drives includes &#123;scenario_id&#125;)</code> | <code>either (a) add a task with requires_acceptance=true and drives including &#123;scenario_id&#125;, or (b) mark the scenario with acceptance_na=&lt;reason ≥5 chars&gt; in spec.md if e2e acceptance is intentionally skipped for this iteration</code> | <code>protocol.md#§5.1</code> |
| <code>VISUAL_CONTRACT_UNBOUND</code> | 2 | <code>spec-lock check 7: visual_contract &#123;visual_id&#125; has no visual-ui task whose visual_contract_refs includes it</code> | <code>either (a) add a visual-ui task with visual_contract_refs including &#123;visual_id&#125;, or (b) mark the visual_contract with visual_na=&lt;reason ≥5 chars&gt; in spec.md if visual verification is intentionally deferred</code> | <code>protocol.md#§5.1</code> |
| <code>TASK_KIND_SCHEMA_VIOLATION</code> | 2 | <code>spec-lock check 8: task &#123;task_id&#125; (kind=&#123;kind&#125;) violates projected kind-specific obligations: &#123;reasons&#125;</code> | <code>amend the task to satisfy its kind contract: structural/docs/spike/chore require no_test_rationale (string ≥10 chars); visual-ui requires visual_contract_refs[] with ≥1 entry. Most commonly surfaces after migration:snapshot_imported when legacy v0.0.x projections lack the required fields. Slice C R2: bug-task RED is execution discipline, not a spec-lock obligation — a behavioral task with labels=['bug'] is born unregistered, and RED registration is enforced at runtime by BUG_TASK_REQUIRES_RED (preflight, implement step) and BUG_TASK_RED_NOT_REGISTERED (verify-accept), never by this check</code> | <code>protocol.md#§5.1</code> |
| <code>GATE_PRECONDITION_VIOLATION</code> | 2 | <code>gate:decided &#123;gate&#125; approval rejected at the mutate layer: &#123;failure_count&#125; check(s) failed</code> | <code>this is a mutate-layer envelope around the underlying gate checks (see detail.checks for the list). spec-lock failure codes: MISSING_VERIFIABILITY / REQ_NOT_DRIVEN / E2E_SCENARIO_UNBOUND / VISUAL_CONTRACT_UNBOUND / TASKS_NOT_PLANNED / TASKS_BASED_ON_STALE / TASK_KIND_SCHEMA_VIOLATION / SPEC_HAS_UNCLARIFIED. verify-accept failure codes: VERIFY_LANE_NOT_PASSED / OPEN_FINDINGS_PRESENT / COVERAGE_NOT_SATISFIED / TASK_DONE_NO_EVIDENCE / SPEC_REVIEW_MISSING / SPEC_REVIEW_IMPLEMENTER_CONFLICT / SPEC_REVIEW_IMPLEMENTER_UNKNOWN / TASKS_NOT_PLANNED (precondition) / TASKS_BASED_ON_STALE (precondition). Fix each listed check then retry the gate decision. Pass 1.5 runs after preflight + reducer dry-run + before sidecar promotion, so a rejected gate batch leaves no on-disk residue.</code> | <code>protocol.md#§5.1</code> |
| <code>MULTIPLE_GATE_DECISIONS</code> | 2 | <code>batch contains &#123;count&#125; approved gate:decided entries (gate_kinds=&#123;gate_kinds&#125;); protocol §10.8 requires one gate decision per atomic operation</code> | <code>split the batch — emit each gate decision as its own mutation. A batch carrying ≥2 gate approvals (even with different gate_kinds, e.g. spec-lock + verify-accept) is not a valid atomic operation. Rejected gate decisions are not counted; only approvals trigger this rule</code> | <code>protocol.md#§10.8</code> |
| <code>GATE_NOT_IMPLEMENTED</code> | 2 | <code>gate=&#123;gate&#125; is not recognized; protocol GateName enum is closed at `spec-lock` or `verify-accept` for v0.1.0</code> | <code>use `loaf gate decide spec-lock` or `loaf gate decide verify-accept`. Future gates beyond v0.1.0 would extend the GateName enum in journal-entry.ts + evidence-schema.ts (lockstep) and wire here.</code> | <code>protocol.md#§10.8</code> |
| <code>VERIFY_LANE_NOT_PASSED</code> | 2 | <code>verify-accept check 1: applicable VERIFY lane=&#123;lane&#125; has no evidence with passing/approved/waived result</code> | <code>add an evidence:added entry with check=&#123;lane&#125; (or a matching kind via the narrow fallback map: local-check/task-summary→run, verify-review/spec-review→review, acceptance→acceptance, visual-review→visual) and result one of `passed`, `approved`, or `waived`. Applicable lanes derive from spec: REQ ⇒ REVIEW, SCEN.tag=e2e ⇒ ACCEPTANCE, VIS ⇒ VISUAL, done task ⇒ RUN+REVIEW.</code> | <code>protocol.md#§5.2</code> |
| <code>OPEN_FINDINGS_PRESENT</code> | 2 | <code>verify-accept check 2: &#123;count&#125; actionable finding(s) still open (ids=&#123;open_ids&#125;); resolve or close before verify-accept</code> | <code>complete the declared action for each listed finding, then run `loaf finding close &lt;FND-id&gt;`; if the honest disposition is carry-forward, raise it with action=defer or action=backlog instead. verify-accept excludes only open findings whose existing action declares deferral</code> | <code>protocol.md#§5.2</code> |
| <code>COVERAGE_NOT_SATISFIED</code> | 2 | <code>&#123;covered_id&#125; has no evidence that satisfies it (canSatisfy failed for all candidates)</code> | <code>add evidence:added covering &#123;covered_id&#125; per protocol §5.4: REQ allows task-summary/verify-review/spec-review/manual+reason/waiver+reason; SCEN.tag=e2e allows acceptance/manual+reason/waiver+reason; VIS allows visual-review+attachment/manual+reason/waiver+reason. Result must be passed/approved/waived per §1035.</code> | <code>protocol.md#§5.2</code> |
| <code>TASK_DONE_NO_EVIDENCE</code> | 2 | <code>verify-accept check 4: task &#123;task_id&#125; is status=done but has no evidence covering it (kind one of `task-summary`, `local-check`, `manual`, or `waiver`)</code> | <code>add evidence:added with covers including &#123;task_id&#125; and kind in the T-allowed set. Most commonly: a task-summary written on closing the task; alternatively local-check (test/lint/typecheck run), manual (human attest), or waiver (human waiver with reason ≥10 chars).</code> | <code>protocol.md#§5.2</code> |
| <code>SPEC_REVIEW_MISSING</code> | 2 | <code>verify-accept check 5: ceremony.strict_spec_review=true requires ≥1 evidence kind=spec-review with result `passed` or `approved` from an actor ≠ implementer; none found</code> | <code>have an independent reviewer (not the implementer of done tasks; not a cli:* automation actor) run a spec review and add an evidence:added with kind=spec-review and result `passed` or `approved`. Note: result=waived does NOT count for spec-review (kind=spec-review + result=waived bypasses the human+reason refine guarantee that kind=manual or kind=waiver provides).</code> | <code>protocol.md#§5.2</code> |
...
| <code>TASK_DEP_NOT_FOUND</code> | 2 | <code>task &#123;task_id&#125; field &#123;field&#125; references missing task &#123;ref&#125;</code> | <code>add the referenced task in the same atomic batch, or amend the dependency to an existing task, then retry</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_DEP_SELF</code> | 2 | <code>task &#123;task_id&#125; cannot depend on itself</code> | <code>remove the self-reference from depends_on, then retry the task graph mutation</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_DEP_DUPLICATE</code> | 2 | <code>task &#123;task_id&#125; repeats dependency &#123;ref&#125; at indexes &#123;indexes&#125;</code> | <code>keep each dependency id only once in depends_on, then retry</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_DEP_CYCLE</code> | 2 | <code>task dependency graph contains cycle &#123;cycle&#125;</code> | <code>remove or redirect one dependency in the reported closed path, then retry</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_DEP_ABANDONED</code> | 2 | <code>task &#123;task_id&#125; field &#123;field&#125; references abandoned task &#123;ref&#125;; &#123;hint&#125;</code> | <code>use an amend-tasks-sponsored task amendment to replace the abandoned dependency, then retry</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_DEPS_NOT_SATISFIED</code> | 2 | <code>task &#123;task_id&#125; cannot be claimed: dependency &#123;blocking_dep&#125; is not done (status=&#123;blocking_status&#125;)</code> | <code>complete deps_on tasks first (run `loaf tasks list --status pending` to see what is blocking), or use `loaf tasks next` to pick a task with all deps satisfied</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_NOT_CLAIMED</code> | 2 | <code>task &#123;task_id&#125; step &#123;step&#125; mutation requires task.status=in_progress (got status=&#123;status&#125;); claim the task first</code> | <code>run `loaf tasks claim &#123;task_id&#125;` to move the task from pending/ready to in_progress before emitting task_step_started or task_step_done; once auto-promoted to done, steps cannot be re-mutated</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_NOT_ABANDONABLE</code> | 2 | <code>task &#123;task_id&#125; cannot be abandoned (status=&#123;status&#125; — already in a final status)</code> | <code>tasks with status=done are already complete and status=abandoned tasks are already abandoned; run `loaf tasks list` to inspect the task graph and abandon a non-terminal task instead</code> | <code>protocol.md#§10.8</code> |
| <code>TASK_ABANDON_BLOCKED_DEPENDENTS</code> | 2 | <code>task &#123;task_id&#125; cannot be abandoned: non-terminal task(s) &#123;blocking_dependents&#125; depend on it; abandon or complete the dependents first</code> | <code>abandon or complete the dependent tasks first (see detail.blocking_dependents), then retry `loaf tasks abandon &#123;task_id&#125; --reason "..."`; abandoning a parent would strand a pending child</code> | <code>protocol.md#§10.8</code> |
| <code>SESSION_REASON_REQUIRED</code> | 2 | <code>&#123;kind&#125;: --reason is required (the session-terminal entry must record why)</code> | <code>re-run with `--reason "..."`; `loaf archive` and `loaf abandon` both require a rationale on the journal entry</code> | <code>protocol.md#§10.8</code> |
| <code>PROJECTION_WRITE_FAILED</code> | 2 | <code>&#123;projection&#125; projection write failed after journal append at last_seq=&#123;last_seq&#125; (spec_version=&#123;spec_version&#125;): &#123;error&#125;</code> | <code>the journal already records the change; do NOT retry the same command. Run `loaf doctor --rebuild` (when available) to resync derived projections from journal truth, or inspect `.loaf/&lt;feature&gt;/journal.jsonl` tail manually.</code> | <code>protocol.md#§10.15</code> |
| <code>FINDING_AMEND_SPEC_NOT_LOCKED</code> | 2 | <code>finding raise action=amend-spec requires state.spec_locked=true; spec is not locked at sub_state=&#123;current_sub_state&#125;, edit directly via `loaf spec submit / add-*`</code> | <code>drop --action amend-spec and use `loaf spec submit` / `loaf spec add-req` / etc. directly while spec is unlocked; amend-spec is reserved for post-`gate decide spec-lock --approve` recovery.</code> | <code>protocol.md#§6.1</code> |
...
| <code>CANONICAL_TASK_BODY_UNAVAILABLE</code> | 2 | <code>task &#123;task_id&#125; is in the projection but has no canonical body in the journal (migration-imported); a whole-task amend cannot be reconstructed</code> | <code>the task was rehydrated from a v0.0.x migration snapshot, so its full body never landed as a journal tasks_planned/tasks_amended entry. Re-plan the task graph via `loaf tasks submit`, or wait for the history-aware doctor path that will reconstruct migrated task bodies.</code> | <code>protocol.md#§10.8</code> |
...
| <code>BUG_TASK_RED_NOT_REGISTERED</code> | 2 | <code>behavioral bug task &#123;task_id&#125; is done but never registered its RED test (red_test_registered≠true)</code> | <code>a done behavioral bug task must have registered its RED test via `loaf tasks register-red`; this is a verify-accept defense-in-depth check for migration / raw-API journals — rebuild the journal or register RED retroactively before re-running the gate.</code> | <code>protocol.md#§9.3</code> |
| <code>SPIKE_CONVERT_NO_SPIKE_TASK</code> | 2 | <code>cannot convert: the session has no non-abandoned spike task; `loaf spike convert` is a spike-task exit (protocol §8.3)</code> | <code>run `loaf spike convert` only from a session that holds a kind=spike task; for a non-spike session close it with `loaf archive --reason "..."` or `loaf abandon --reason "..."`</code> | <code>protocol.md#§8.3</code> |
| <code>SNAPSHOT_STALE_REBUILD_REQUIRED</code> | 2 | <code>snapshot stale (reason=&#123;reason&#125;) at &#123;feature_dir&#125;; run `loaf doctor --rebuild --feature &lt;feature&gt;` to re-serialize from journal truth</code> | <code>snapshot meta/leaves no longer agree with the journal tail; run `loaf doctor --rebuild --feature &lt;feature&gt;` to re-serialize from journal truth, then retry. Inspect detail.reason + reason-specific fields (meta_path / projection_kind / cause) to triage corruption source before rebuilding.</code> | <code>protocol.md#§10.15</code> |
| <code>JOURNAL_TAIL_REQUIRES_NEWER_LOAF</code> | 2 | <code>tail recovery refused at seq &#123;seq&#125;: journal kind &#123;kind&#125; uses entry schema &#123;entry_schema_version&#125; (&#123;reason&#125;)</code> | <code>preserve journal.jsonl byte-for-byte and upgrade loaf to a version that understands this entry before running tail recovery again</code> | <code>protocol.md#§10.15</code> |
| <code>INVALID_PRESET</code> | 2 | <code>invalid ceremony preset</code> | <code>Use one of quick, light, standard, or deep.</code> | <code>protocol.md#§10.5</code> |
| <code>USAGE</code> | 2 | <code>invalid CLI usage</code> | <code>Run the command with --help and retry with the required flags/arguments.</code> | <code>protocol.md#§10.5</code> |
| <code>DOCTOR_MODE_NOT_IMPLEMENTED</code> | 2 | <code>requested loaf doctor mode is not implemented in this release</code> | <code>Use loaf doctor --rebuild --feature &lt;name&gt;; other doctor modes are deferred.</code> | <code>protocol.md#§10.15</code> |
| <code>DOCTOR_FEATURE_REQUIRED</code> | 2 | <code>loaf doctor --rebuild requires --feature &lt;name&gt;</code> | <code>Pass --feature &lt;name&gt; or --feature-dir &lt;path&gt; for the session to rebuild.</code> | <code>protocol.md#§10.15</code> |
| <code>DOCTOR_REBUILD_FAILED</code> | 2 | <code>doctor --rebuild failed</code> | <code>Inspect the emitted error message; fix the journal/projection issue, then rerun doctor --rebuild.</code> | <code>protocol.md#§10.15</code> |
| <code>DOCTOR_REBUILD_MIGRATED_UNSUPPORTED</code> | 2 | <code>doctor --rebuild does not support v0.0.x-migrated journals in this release</code> | <code>Use the existing migrated snapshots, or wait for migrate-v2/rebuild support.</code> | <code>protocol.md#§10.15</code> |
| <code>REDUCER_ERROR</code> | 2 | <code>internal reducer invariant failed</code> | <code>Preserve the journal and command stderr; this indicates a loaf-cli bug or inconsistent projection state.</code> | <code>protocol.md#§10.5</code> |
...
- `loaf tasks list/next/step/submit/add/claim/abandon/complete/amend/register-red` ✅(`tasks check` 命名锁定但 **inventory:future,未在 v0.1.0 实装**)
...
| `--dry-run` | `-n` | bool | false | **rev 4.1**:mutating 命令只校验不落盘(见下方 dry-run 契约);read-only 命令 reject + exit 2。**当前实现**:mutator 获取与 live run 相同的 owner-fenced feature lease，在 lease 内确认 caller-loaded meta 仍匹配 journal tail，再走 Pass 0/1/1.5 + Pass A 上下文完整性检查并短路；不 materialize sidecar、不 append、不 rebuild projection。`MutateContext.dryRun` 控制开关;read-only 命令 reject 走 typed diagnostic `DRY_RUN_NOT_APPLICABLE`;trace.jsonl 在 dry-run 下被抑制;`loadSession` 跳过 mkdir 避免 feature 目录侧效。`spec edit --input` 走 mutator dry-run且不改 work copy；无 `--input` 的 editor wrapping lane 与 `tui` reject |
| `--session <UUID>` | — | string | `$LOAF_SESSION` env or auto-pick | **rev 4.1**:dispatch 单次覆盖,见 §10.3 precedence;接受 ≥8 字符 prefix。**v0.1.0 当前实现(Phase 16 SC-8)**:`resolveDispatch` 5-level precedence(`--session` > `--feature` > `$LOAF_SESSION` > `$LOAF_FEATURE` > auto-pick);prefix `<8` chars → USAGE;UUID 匹配 0 个 → `SESSION_NOT_FOUND`(new diagnostic SC-8);组合 `--feature-dir` → USAGE(pre-parse 拦截);registry 读取尊重 `LOAF_REGISTRY_DIR` env(SC-7 test isolation hook);cwd 比较走 `fs.realpath` canonicalize(macOS `/var/folders` vs `/private/var/folders`)|
...
| `--input <src>` <!-- inventory:placeholder reason="per-command flag documented in global section; SC-3 InputSourceResolver + SC-4a/b/c unify modality across mutators" --> | — | `-\|<json>\|<path>` | — | **rev 4.3**(ADR-0004 A3/A11)+ **Phase 16 SC-3/SC-4a**:Tier 1 mutator 命令的统一 JSON 输入 source(本行只规定 source 解析,不规定 payload shape — shape 由每个命令自己的 input schema 决定)。`-` ⇒ stdin;首字符 `{` 或 `[` ⇒ inline JSON;其余 ⇒ 文件路径(不存在 → exit 2 `INPUT_FILE_NOT_FOUND`)。**Payload shape per 命令**:`spec submit` 仅接 single object(whole-replacement);`spec edit` 接 strict `{"body":"<Markdown>"}` 且只替换 body、保留当前 frontmatter;`tasks submit` 接 strict `{"tasks":[semantic-task,...]}` single object;5 个 batch-capable mutator(`spec add-req` / `spec add-scenario` / `spec add-visual` / `tasks add` / `evidence add`)接 single object 或非空数组(batch,A10)。判别细节见下方「`--input` source 判别」 |
| `--schema` <!-- inventory:placeholder reason="per-command modifier on 6 structured authoring commands; not a top-level global flag" --> | — | bool | false | **rev 5.2 / A08** modifier:与 `--format=json` 联用(`loaf <cmd> --schema --format=json`)dump 该命令 input Zod schema 派生的 JSON Schema(查 `src/cli/input-schemas.ts::INPUT_SCHEMAS`)。实装范围为 6 个 structured authoring commands:`spec add-req` / `spec add-scenario` / `spec add-visual` / `tasks submit` / `tasks add` / `evidence add`;其中 `tasks submit` 是 strict single object,其余 5 个是 batch-capable single-or-array。其它命令传 `--schema` → 由 Commander 报 `unknown option`。**artifact projection schema**(5 个 kind:`spec` / `tasks` / `evidence` / `finding` / `state`)不走 `--schema` modifier,改走 literal `loaf <kind> schema` 子命令(§10.8 例:`loaf evidence schema --format=json`)— 两条 surface 是有意分叉,modifier 给 input shape,subcommand 给 artifact shape。用于 LLM 自描述 / fixture 生成 / `SCHEMA_VALIDATION_FAILED` 修复提示 |

<!-- inventory:current-end -->


**Format flag 归一化与互斥**(rev 4.2,clig.dev §6):
- 内部归一到 single `format`:`--plain` ⇒ `format=text`(future SC-5b),`--format=<fmt>` 显式赋值
- precedence(high → low):显式 flag > `$LOAF_FORMAT` env(v1.0,见 §10.3)> TTY-derived default(stdout TTY → text;pipe → text line-oriented)
- **同值无冲突**(future,SC-5b 引入 `--plain` 后激活):`--plain --format=text` OK
- **冲突值**(future,SC-5b 引入 `--plain` 后激活)→ exit 2 `MUTUALLY_EXCLUSIVE_FLAGS`,stderr 列冲突 flag 对:`--plain --format=json`
...
TTY no-hang 守卫见 §10.1(stdin TTY + `--input -` → USAGE exit 2,不 hang)。`MISSING_INPUT` 覆盖 stdin 读失败 + 缺 flag 两种(Phase 16 SC-4b 扩,见 §10.5 / `src/core/error-catalog.ts`)。

**`--help` 顶部约定**(clig.dev §5):每个接受 `--input` 的命令 `--help` 顶部带 2-3 个工作 JSON 示例(覆盖典型 shape),flag 表在示例下方。`loaf <cmd> --schema --format=json` 一并 dump JSON Schema 供 LLM / fixture 自描述。
...
| 命令类别 | `--dry-run` 行为 |
|---|---|
| **Mutating 命令**(`advance` / `spec submit` / `tasks submit` / `tasks step start/done` / `tasks amend` / `evidence add` / `finding raise/close` / `gate decide` / `waive` / `settle` / `archive` / `abandon` / `spike convert` / `pending resolve` / `profile escalate` / `lessons add` / `start` / **`resume`** _(rev 5.0 / Phase 16 SC-13b — emit `session:resumed` no-op marker)_) | **rev 5.0**:走 §11.2 10-step transaction 步 1-5(acquire .lock → read tail + `_meta` fast-check → preflight validate → prepare sidecar files into short-lived `.tmp-*`(不 rename)→ final validate against embedded refs),然后 **跳过 step 6 journal append + step 7 post-apply assert + step 8 snapshot rebuild + step 9 registry refresh**,改为:**unlink sidecar `.tmp-*` + release .lock**(即 step 10 cleanup-only 分支)。stdout 打"would do"摘要(JSON / text 按 `--format`)。**不分配 EV/LSN/PEND/T/REQ/SCEN/VIS id**(避免单调计数器空跳);若校验过则 stdout 列将分配的 next-id 范围。**不写 journal entry、不 rebuild snapshots、不 refresh registry**。exit 0 = 校验通过,exit 2 = 会失败(schema / transition / actor authority / lock 抢占等)。机器表达见 `src/core/concurrency-contract.ts::dry_run_transaction_order` |
| **Read-only 命令**(`status` / `spec status` / `journal list`(`log` alias) / `tasks list` / `tasks next` / `tasks complete` / `evidence list` / `finding list` / `pending list` / `pending status` / `verify status` / `check <path>` / `<artifact> schema --format=json` / `evidence schema` / `sessions list` / `tui` / `doctor`(含 `--rebuild`,v0.1.0 reject;in-memory replay precheck future)/ `--version` / `--help`;`tasks check` 为 inventory:future,未实装)| **reject** `--dry-run`:exit 2 `DRY_RUN_NOT_APPLICABLE`(typed diagnostic;text 模式 stderr 渲染 `error: DRY_RUN_NOT_APPLICABLE — --dry-run not applicable to read-only command \`<command>\``;json 模式 ok:false 结构,`detail.command` + `detail.command_type` 携带 `"read-only"`)|
...
**rev 4.1 invariant**:dry-run 命令**不持久任何状态**(不写 `.loaf/<feature>/*`、不改 `~/.loaf/registry/`、不递增 EV-id 计数器)。机器表达见 `src/core/concurrency-contract.ts::CONCURRENCY_INVARIANTS.dry_run_transaction_order`。

### 10.8 Command table

**命名修正**(对照 rev 3.x):
- `loaf check tasks` → 名词在前的规范名为 **`loaf tasks check`**(consistency:`loaf tasks` 系列名词在前)。**注**:`loaf tasks check` 为 inventory:future,未在 v0.1.0 实装;SC-17 起,对 `loaf check tasks` 误用的运行时 did-you-mean(§1899)**改指 v0.1.0 可用的 path-based 形式** `loaf check <path>/tasks.json --kind tasks`,不再指向未实装命令
...
> **rev 4.1 did-you-mean 兜底(SC-17 更新)**:用户敲 `loaf check tasks`(把 "tasks" 当 enum keyword 误用)时,`loaf check <path>` 在 path 解析阶段发现 "tasks" 不是有效文件路径,走 §10.1 did-you-mean 规则,stderr 提示改用 v0.1.0 可用的 path-based 校验 `loaf check <path>/tasks.json --kind tasks` 并 exit 2。**`loaf tasks check`(inventory:future,未实装)与 `loaf check <path>` 不是 canonical/alias 关系**:前者(未来命令)跑 `tasks.execution.status ↔ evidence.jsonl` 一致性专项校验,后者是单文件 schema 校验入口(CI 用),做的不是同一件事。**SC-17 起**,did-you-mean **不再指向未实装的 `loaf tasks check`**,改指 path-based 形式(避免把用户引向 exit-2 unknown-command 的死胡同)。

**Tier 1 mutator 通用契约**(rev 4.3,ADR-0004 A2 / A3 / A5 / A10):

- `spec submit` + `tasks submit` + 5 个 batch-capable structured mutator(`spec add-req` / `spec add-scenario` / `spec add-visual` / `tasks add` / `evidence add`)走统一 `--input <-|json|path>` modality(§10.7);positional id / per-field flag 已砍。`spec submit` 与 `tasks submit` 仅接各自的 strict single object(whole-replacement);其余 5 个支持 single 或 array(batch)。
...
- `id` 由 CLI 在 per-session lock 内单调分配:REQ / SCEN / VIS 输入只携 `id_namespace`(stem),CLI 扫该 namespace 下 max serial → 拼完整 id 落 spec.md(input regex vs output regex 见 §4.2 ID 分配段 + `src/core/spec-schema.ts`)。T / EV / FND / PEND 完全 CLI 自动分配,input **不**传 namespace 字段。
- `--schema` 全局 modifier 跟 `--format=json` 联用(`loaf <cmd> --schema --format=json`)dump 该 input 的 JSON Schema(派生自 `src/cli/input-schemas.ts::INPUT_SCHEMAS`)。
- shape enforcement 全在 CLI:三选一可验证 / 6×6 finding grid / attachment hash+mime / id 唯一性 — workflow content 留给 loaf-skill,见 §19。
- 下表 command signature 列出各命令的**本地** positional/options；§10.7 global flags 对所有命令继续适用，不在每行重复。

<!-- inventory:current-begin v0.1.0 commands -->

| 命令 | 用途 | exit |
|---|---|---|
| `loaf prune restore <session-id> [--at <value>]` | 从 recoverable trash 恢复指定 session；`--at` 用于同一 id 多个 trash bucket 时消歧 | 0 / 2 |
| `loaf start <feature> [--ceremony <value>] [--label <value>] [--workspace <value>] [--feature-dir <value>]` | 进入 TRIAGE,在 `.loaf/<feature>/` 起新 session。**rev 4.1**:stdout **最后一行**打印新 session 的 UUID(可预测,shell 脚本可 `UUID=$(loaf start ... \| tail -1)` 抓取);stderr 一行 state-change(§10.12)。**Phase 15 SC1**(F-019):`--label` / `--workspace` 连同 `--ceremony` preset 名一并写入 `session:started` payload 的 bucket-C identity 字段(`session_label` / `workspace` / `ceremony_label` / `loaf_version_required`),使 `state.json` 投影全 journal 派生 | 0 / 2 |
| `loaf status [--feature <value>] [--feature-dir <value>]` | 读 state + artifact 健康度。**Phase 15 SC3**:read-path 切到 `projection-loader`(snapshots/_meta.json fast-check 后读 `snapshots/*.json`,9-reason stale 见 §10.15)。pre-`loaf start` dir 现 exit 2 `NO_SESSION`(对齐 `tasks list` / `pending list` / `finding list`;改自 rev 4.x exit 0 + `state:null` 软探针;codex r175a)| 0 / 2 |
| `loaf journal list [--after-seq <value>] [--limit <value>] [--kind <value>] [--actor <value>] [--feature <value>] [--feature-dir <value>]` | **ticket #12 observability v1**:read-only canonical journal timeline;`loaf log` 是 Commander alias。按 journal 顺序输出 envelope-only rows；JSON envelope exact keys=`{ok,feature,count,entries}`，row exact keys=`{seq,entry_id,at,actor,kind}`，存在 atomic batch marker 时追加且只追加 `{batch_id,batch_index,batch_count}`。永不输出或解释 per-kind `payload`。`--after-seq` 接非负整数且为 exclusive lower bound；`--limit` 接正整数；`--kind` 对 `KIND_REGISTRY` closed set 校验；`--actor` 做 prefix-or-full `startsWith` 匹配。text 每 entry 一行；空集 exit 0。`--dry-run` reject | 0 / 2 |
| `loaf next [--feature <value>] [--feature-dir <value>]` | **read-only**:从 cursor + ceremony + pending head + VERIFY applicability 计算确定的下一条 owner command(`advance` / `deliver` / `settle` / `gate decide` / `tasks next` / `pending resolve` / `profile escalate`)。`buildScopedNextOutput` 为 `loaf next` 与 mutation success advisory 共用同一 core routing 结果和 selector renderer；全局 `--session` dispatch 保留 canonical UUID，显式 feature-dir 保留路径，其余保留 feature selector。仅 gate / pending / human-input stop 输出 `blocked=true`;terminal state 省略 `next_action` | 0 / 2 |
...
| `loaf spec init [--feature <value>] [--feature-dir <value>] [--feature-id <value>] [--feature-name <value>] [--intent <value>]` | 生成 spec.md 模板,适合 `$EDITOR` 跟进 | 0 |
| `loaf spec edit [--input <value>] [--feature <value>] [--feature-dir <value>]` | **Phase 16 SC-12a + issue #18A**:`--input` 接统一 source resolver 与 strict JSON `{body:string}`，保留当前 frontmatter、只替换 Markdown body，完全跳过 `$EDITOR`；file / stdin `-` / inline JSON 三 lane 等价。无 `--input` 时保留 wrapping mutator：仅当 stdin **和** stdout 均为 TTY 才起显式 `$EDITOR`（无显式值 fallback `vi`）on `<feature-dir>/spec.md`，save 后 reload + validate；任一 stream 非 TTY 均 `SPEC_EDIT_INPUT_REQUIRED`，不受 `$EDITOR` 是否配置影响且不 spawn。两 lane 都 emit `event:spec_submitted` batch（复用 `buildSpecSubmitBatch`），内容无变化 → `no_op:true` 不写 journal。`--input --dry-run` 走标准 mutator dry-run且不改 work copy；editor lane `--dry-run` 仍 reject `DRY_RUN_NOT_APPLICABLE(command_type=wrapping)`。失败 work copy 语义、frontmatter subcode、CLI-owned spec_version、signal/editor exit/tokenize 与 post-lock gate 均保持既有 SC-12a 契约。| 0 / 2 |
| `loaf spec add-req [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>]` | **rev 4.3**(ADR-0004 A1 / A4 / A5):增量加单条或 batch EARS REQ。Input 含 `id_namespace`(`^REQ-[A-Z][A-Z0-9]*$`)+ EARS 字段;CLI 在 lock 内拼完整 `id`(`REQ-<NS>-<NNN>`)落 `spec.md`,`spec_version += 1`(per invocation,A10)。pre-lock(SPEC.spec/plan/design)合法;post-lock 拒 → `SPEC_LOCKED_NO_DIRECT_EDIT` exit 2,走 `amend-spec` finding。`--schema --format=json` dump input JSON Schema | 0 / 2 |
| `loaf spec add-scenario [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>]` | **rev 4.3**(ADR-0004 A1 / A4 / A5):增量加 Gherkin scenario,id_namespace 模式 `^SCEN-[A-Z][A-Z0-9-]*$`。其余规约同 `spec add-req` | 0 / 2 |
| `loaf spec add-visual [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>]` | **rev 4.3**(ADR-0004 A1 / A4 / A5):增量加 visual contract,id_namespace 模式 `^VIS-[A-Z][A-Z0-9-]*$`。其余规约同 `spec add-req` | 0 / 2 |
| `loaf tasks submit [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>]` | 提交完整 task graph(SPEC.design / EXECUTE.plan)。**A08 strict semantic authoring**:`--input` 是 single `{"tasks":[...]}`;每条 task 带 unique `local_key`,dependency refs 显式用 `{local_key}` / `{task_id}`,不得带 `id/status/execution/based_on`。CLI 在 feature lease 内 two-pass 分配历史不复用的 T-id、解析 forward refs、从 current snapshot 注入 `based_on.spec`,emit wire-compatible full `event:tasks_planned`;JSON 成功结果含 `task_ids_by_local_key`。旧 full task plan input 明确 breaking reject,但历史 journal replay 不变。`--schema --format=json` dump exact input schema。source modality 为 stdin / inline JSON / path(§10.7)。 | 0 / 2 |
| `loaf tasks add [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>] [--finding <value>]` | **rev 4.3** + **rev 5.0** + **A08**:单条或 batch semantic task 加入,与 `tasks submit` 共用 `local_key` / explicit dependency-ref contract。`SPEC.design` 阶段(无 `--finding`)emit 整图 whole-replacement `event:tasks_planned`;`EXECUTE.work` 阶段(带 `--finding <FND-N>`)是 sponsored 路径 —— 每个新增 task emit 一条 `event:tasks_amended` `mode=add` + `sponsored_by_finding_id`。两个路径都在 feature lease 内 two-pass allocate,成功 JSON 回传 `task_ids_by_local_key`。`--finding` 在 `SPEC.design` → `USAGE` 拒绝;无 `--finding` 在非 `SPEC.design` → `SUB_STATE_AUTHORITY_VIOLATION`。`--input <src>` 接统一 modality且支持 single object / non-empty array;`--schema --format=json` dump exact input schema。 | 0 / 2 |
| `loaf tasks claim <task-id> [--feature <value>] [--feature-dir <value>]` | 把 task 从 `ready` 拉到 `in_progress`(worker 拿活,fan-out 多 worker 并发用);CLI 在 lock 内确认 deps_on satisfied | 0 / 2 |
| `loaf tasks abandon <task-id> --reason <value> [--feature <value>] [--feature-dir <value>]` | **Item 1**:把非终态 task 标记为 `abandoned`(EXECUTE.work 阶段;`--reason` 必填,emit `event:task_abandoned` payload 含 task_id + reason)。已终态(`done` / `abandoned`)→ `TASK_NOT_ABANDONABLE` exit 2;若有非终态 task 在 `depends_on` 指向该 task → `TASK_ABANDON_BLOCKED_DEPENDENTS` exit 2(放弃父任务会孤立子任务)。配合 `EXECUTE_DONE_TASKS_NOT_FINAL`:超出范围的 task 可 abandon 后再 `loaf advance EXECUTE.done` | 0 / 2 |
| `loaf tasks complete <task-id> [--feature <value>] [--feature-dir <value>]` | 确认 task 已达 `done`(全部 must step 已 passed/waived/na)。**rev 5.0 / Slice C SC-C1**:`event:task_step_done` 已在末个 must step 完成时 auto-promote `task.status=done`,故 `tasks complete` 是**只读确认命令**——不 emit journal entry;task 已 `done` → exit 0,否则 `TASK_COMPLETE_PRECONDITION_VIOLATED` exit 2 并列出未达 terminal-positive 的 must step。**rev 4.2**:rename from `loaf tasks done` — 与 `tasks step done` 同名异级歧义,改 `complete` 消歧(clig.dev §8) | 0 / 2 |
| `loaf tasks register-red <task-id> [--feature <value>] [--feature-dir <value>]` | 给已有失败 RED test 的 `behavioral+labels=["bug"]` task 记录 implement 前 ordering proof;不是通用 step shortcut(§9.3 唯一硬约束)| 0 / 2 |
| `loaf tasks amend <task-id> [--feature <value>] [--feature-dir <value>] [--policy <value>] [--input <value>] [--finding <value>]` | 两条互斥 surface。`--policy`(spec-lock 后 EXECUTE.plan 阶段)窄修 `execution[].applicability`,不能改 drives / depends_on / kind(见 §8.6)。**Item 3 SC1b**:`--input <src> --finding <FND-N>` 是 sponsored 结构化 graph 替换 —— 回退到 `EXECUTE.work` 后用新 id-less task 定义替换该 task 的 graph,emit `event:tasks_amended` `mode=replace` + `sponsored_by_finding_id`;CLI 以 `materializeTaskForAmend` 把 current runtime 进度叠加到新 graph,preflight §8.6 sponsored 分支校验 finding + frozen-field 红线。`--policy` 与 `--input` 同时给 → `USAGE` 拒绝。**Phase 16 SC-4b**:`--input <src>` 接统一 modality(`-` stdin / inline JSON / file path,§10.7);**single id-less task object only**(NOT batch-capable;sponsored single-task replacement)。 | 0 / 2 |
| `loaf tasks list [--feature <value>] [--feature-dir <value>] [--status <value>]` | 列所有 task 当前 step(rev 4.0:rename from `loaf tasks status`,避免跟 `loaf status` session-level 命名歧义) | 0 |
| `loaf tasks next [--feature <value>] [--feature-dir <value>]` | CLI 算下一个 ready task | 0 |
...
| `loaf tasks step start --task <value> --step <value> [--feature <value>] [--feature-dir <value>]` | 开始一个 step(运行时校验 step ∈ kind 合法集) | 0 / 2 |
| `loaf tasks step done --task <value> --step <value> [--result <value>] [--evidence-kind <value>] [--evidence-result <value>] [--evidence-summary <value>] [--evidence-covers <value>] [--evidence-check <value>] [--evidence-reason <value>] [--evidence-actor <value>] [--feature <value>] [--feature-dir <value>]` | 完成一个 step。`--result` 是 step outcome,独立于 `--evidence-result` 的 evidence outcome(RED reproduction 完成可 step=`passed` + evidence=`failed`)。**rev 5.0** 行为(等价于原 rev 4.1 contract):走 §11.2 10-step journal transaction,**同一 batch 内 emit** `event:task_step_done` + 可选 `evidence:added`(若 `--evidence-*` flag);step 5 final-validate 通过后整批 append,reducer 派生到 `snapshots/tasks.json`(`execution.<step>.status`)与 `snapshots/evidence.json`。无 `--evidence-*` 时仍可单条 append;缺 task passing evidence 不在 step-done preflight 拦截,后续由 quick/light `loaf deliver` 的 verify-min 或 standard/deep `verify-accept` 拦截;未来 `loaf tasks check`(F-023) 才使用 `TASK_STATUS_WITHOUT_PROOF` 做 execution↔evidence 一致性诊断 | 0 / 2 |
| `loaf evidence add [--input <value>] [--schema] [--feature <value>] [--feature-dir <value>]` | **rev 5.0**:走 §11.2 transaction,**emit `evidence:added`**(单条或 batch,batch markers N≥2);reducer 派生到 `snapshots/evidence.json`(含 covers[] / check / actor / result)。**issue #20**:成功写入后对每个 `covers[]` pair 复用 §5.4 `canSatisfy` 检查；不兼容则 stderr warning 列 obligation id、supplied kind、allowed kinds，但不 reject、不改 JSON stdout exact keys。**rev 4.1**:不接受 `--id` flag,EV-id 由 CLI 单调分配在 payload 内并 stdout 回打;支持 `external_ref` 字段留调用方 correlation。**rev 4.3**(ADR-0004 A3 / A10):走统一 `--input <src>` modality(`-` stdin / inline JSON / file path,§10.7),**Phase 16 SC-4c** 接入 + 启用 batch(single object 或 non-empty array)。**ADR-0004 A6 deferred(NOT in SC-4c)**:`attachments` 简化形态 `[{path}]` + CLI 自动 sha256/mime/bytes materialization 留给后续 SC;**当前** runtime + `INPUT_SCHEMAS["evidence:add"]` machine schema 都要求 caller 提供完整 `Attachment` 形态 `[{path, sha256, mime, bytes?}]`(见 §4.4 命令行 sha256/mime 计算示例)。caller 显式传 envelope 字段(`id`/`evidence_id`/`schema_version`/`at`)→ `SCHEMA_VALIDATION_FAILED`(codex r230 PATCH D / r234)。当 A6 落地时,简化形态会在 schema 切换前先做 materialization → `ATTACHMENT_NOT_FOUND` / `ATTACHMENT_NOT_FILE` 路径才会重新激活 | 0 / 2 |
| `loaf evidence list [--covers <value>] [--task <value>] [--kind <value>] [--feature <value>] [--feature-dir <value>]` | **ticket #12 observability v1**:read-only coverage query over freshness-checked `snapshots/evidence.json`。JSON envelope exact keys=`{ok,feature,count,evidence}`；row exact keys=`{id,kind,covers,task_id,at,actor}`，无 task linkage 时 `task_id:null`，不透传 whole evidence payload / summary / result。`--covers` 经 `CoversRefPayload` closed union 校验并匹配 `covers[]`；`--task` 经 `TaskIdPayload` 校验并匹配 `task_id`；`--kind` 经 `EvidenceKind` closed enum 校验。filters AND-compose，保持 projection/journal 顺序；text 每 evidence 一行；空集 exit 0。`--dry-run` reject | 0 / 2 |
| `loaf evidence schema` | **Phase 16 SC-10**:dump evidence projection JSON Schema(派生自 `EvidenceJson`,zod v4 内置 `z.toJSONSchema()`,JSON Schema draft-2020-12)。属于 5 个 `loaf <kind> schema` artifact 子命令家族之一(spec / tasks / evidence / finding / state — pending 不在范围,§1947 closed enum)。read-only,`--dry-run` reject。feature-agnostic:`--feature` / `--feature-dir` / `--session` / `$LOAF_*` 全 pre-parse 拒绝 | 0 / 2 |
| `loaf waive <obligation-id> --reason <value> [--feature <value>] [--feature-dir <value>]` | **rev 5.0 / Phase 16 SC-11**:sugar wrapper over `evidence:added` payload.kind=`waiver`。`<obligation-id>` 通过 `CoversRefPayload` 校验(REQ-/SCEN-/VIS-/T-),同时进 `covers=[id]` 和 `waiver_obligation_id=id`(双字段方案 Plan A)。`actor` 必须 `human:*`(行不通的非 human ctx 报 `NO_HUMAN_ACTOR`),`--reason` ≥10 字符(CLI 友好 USAGE 在 EvidenceFullPayload refine 之前预校验)。`result=waived` 强制。CLI 通过共享 `allocateNextEvidenceId(snapshot)` 分配 EV-id 并 stdout 回打;single-shot(非 batch);`--dry-run` 走标准 mutator 校验路径不写 journal | 0 / 2 |
| `loaf finding raise --category <value> --action <value> [--summary <value>] [--reason <value>] [--target-task <value>] [--target-step <value>] [--feature <value>] [--feature-dir <value>]` | **rev 5.0**:emit `finding:raised`;若 action 触发 back-edge,同 batch 加 emit `event:phase_advanced`(`amend-spec` → SPEC.spec;`amend-tasks` → EXECUTE.work);`fix-impl` / `fix-test`(Phase 11 Item 3 SC2-SC3)再额外 emit `event:task_step_reset`(把目标 step 重置为 `pending`),整批 `[finding:raised, event:task_step_reset, event:phase_advanced]`;reducer 派生到 `snapshots/findings.json` | 0 / 2 |
| `loaf finding list [--feature <value>] [--status <value>] [--feature-dir <value>]` | 列 findings | 0 |
...
| `loaf hook <event> [--list-events] [--feature <value>] [--feature-dir <value>] [--session <value>] [--path <value>]` | Claude Code hook 入口。`<event>` enum 限定 4 值(`session-start` / `write-guard` / `scope-track` / `closure-check`,详见 §11);`bare loaf hook` → exit 2 USAGE 列出 enum(`detail.events`);未知 event → exit 2 USAGE + did-you-mean;`--list-events` 显式 dump(text mode 每行 `<event>\t<ClaudeCodeName>`,JSON 模式 `{ok:true, count, events:[{event, claude_code}]}`)。**SC-15b 已实装 read-side**:`session-start` 经 §10.3 dispatch precedence 解析 feature,读 state/findings/pending 投影,stdout 输出 Claude Code SessionStart hook 形 `{hookSpecificOutput:{hookEventName, additionalContext}}`(注入 sub_state 的 `prompt_inject` + open findings + iteration + head pending),无 active session(非 loaf 工程 / 空 .loaf / stale)静默 exit 0;`closure-check` 读 state/tasks/evidence/findings 跑 read-only 一致性检查(projection freshness via loader fast-check + orphan evidence covers[] task target + open findings 摘要)：actionable open finding 作为 warning；`defer` / `backlog` 另列 `deferred findings carried` 信息并保留 action；stderr 输出但**永远 exit 0**(不阻塞 Stop)。**SC-15c 已实装 write-side**:`write-guard`(PreToolUse)经 strict 路径解析(`--path` 优先,否则 non-TTY 解析 Claude Code hook stdin JSON `tool_input.file_path`,TTY 无 `--path` → USAGE),对 (sub_state.write_paths ∪ active task/step `STEP_WRITE_PATHS_BY_KIND` ∪ `VERIFY_CHECK_WRITE_PATHS`) 经 `loaf.config.json paths.*` **按语义类别定向放大**(`src/core/step-write-paths.ts::STEP_WRITE_CATEGORIES_BY_KIND` 决定哪些 `paths.*` key 可放大当前 step,绝非 flat union)后用 picomatch(`dot:true`)匹配;`protected_files` 硬 deny。allow → exit 0;outside → exit 2 `WRITE_PATH_VIOLATION`;protected → exit 2 `PROTECTED_FILE_WRITE`(纯 exit-code 信号,非 JSON envelope)。安全极性:无 loaf session(非 loaf 工程 / `FEATURE_NOT_FOUND`)→ allow exit 0,但选中 session stale/corrupt 或 config 非法 → **fail closed** exit 2(与 read-side 相反)。**ticket #11 SC3**:`scope-track`(PostToolUse)复用 strict path/stdin 入口；filesystem-aware canonicalization 解析 repo/target realpath，missing leaf 从 nearest existing ancestor 重建，symlink-outside/lexical-outside fail closed。每次 selected-session invocation 通过专用 runtime lock 刷新 heartbeat；仅 `EXECUTE.work` 按 iteration union `pending_scope`，`.loaf/**` 与其它 cursor heartbeat-only。无 session 静默 0；selected stale/corrupt、containment 或 runtime failure exit 2；stdout 恒空；journal emission 延后到 closure command。`HOOK_EVENT_NOT_IMPLEMENTED` 在 runtime 不再触发,catalog 留 reserved-for-future-events(参 `TASK_STATUS_WITHOUT_PROOF` 模式)| 0 / 2 |
| `loaf pending raise --kind <value> --question <value> [--options <value>] [--task-id <value>] [--feature <value>] [--feature-dir <value>]` | **rev 4.1**:append 新 pending 到队列尾(skill / hook / sub-agent 内部调用,user 通常不直接敲);CLI 分配 PEND-id 并 stdout 回打;5 种 kind 任选(`ask_user_question` / `gate_decision` / `spec_clarification` / `finding_decision` / `profile_escalation`)。**不接受 `--id` flag**(同 `evidence add` 纪律,见 §11.2 + `src/core/concurrency-contract.ts`) | 0 / 2 |
| `loaf pending list [--feature <value>] [--feature-dir <value>]` | 列 state.pending FIFO 队列全部 entry(**rev 4.1**:含 head + 排队的;head 标 `*`)| 0 |
| `loaf pending status [--feature <value>] [--id <value>] [--feature-dir <value>]` | 查 single entry 详情(**rev 4.1**:default 看 head;`--id` 看队列其它位置 — 只读不动队列)| 0 / 2 |
| `loaf pending resolve --answer <value> [--feature <value>] [--feature-dir <value>]` | 回答 pending head(**rev 4.1**:FIFO 严格 — 永远 pop `pending[0]`,**不接受 `--id`**;v1.0 不支持跳序)。**v0.1.0**:`--answer` 是 Commander `requiredOption`,任何 stdin/TTY 状态缺 flag → USAGE exit 2;未来 prompt 入口落地后,`--no-input` 强制 fail-closed,TTY + 无 `--no-input` 展示 head 的 `question` + `options` 走 prompt(见 §10.7)。resolve 成功后队列若仍非空,新 head 自动 promote 为 blocker | 0 / 2 |
| `loaf doctor [--rebuild] [--feature <value>] [--feature-dir <value>]` | 版本 / 装机 / 仓库结构自检 + 修复建议。**rev 5.0** 加 5 sub-flag:`--rebuild`(full replay 重建 snapshots/)/ `--check-tail`(只跑 batch-aware tail recovery)/ `--migrate-v2`(v0.0.x → v0.1.0 sidecar import,§5.2)/ `--scope cwd`(对 cwd 下所有 `.loaf/<feature>/` 跑 mixed-version check)/ `--verify-checksum`(full chain rolling_checksum recompute,O(N))。**实现状态(Phase 14 / `1d6e1d1`)**:当前 binary 仅 ship `loaf doctor --rebuild --feature <X> [--feature-dir <path>]`(full replay 重建 snapshots/);bare `loaf doctor` 与其余 4 sub-flag 未实现 → exit 2 `DOCTOR_MODE_NOT_IMPLEMENTED`。详 §10.15 + ADR-0005 §5.4 | 0 / 1 / 2 |
...
| `loaf spec submit --input <src>` | atomic batch:`event:spec_submitted` (batch_index=0,reducer 重置 projection arrays) + companion `event:spec_req_added` × N / `event:spec_scenario_added` × M / `event:spec_visual_added` × K (batch_index≥1,共享同一 batch_id + spec_version)。整批保证 journal 可 replay 回 `spec.md`(canonical truth,§4.2 derived projection) |
| `loaf spec edit` | 保存有效 work copy 后复用 submit builder，atomic batch:`event:spec_submitted` + `event:spec_req_added` × N + `event:spec_scenario_added` × M + `event:spec_visual_added` × K；no-op 不写 journal |
| `loaf tasks submit` | `event:tasks_planned` |
...
| `loaf gate decide` | `gate:decided` + atomic `pending:resolved` 消 head；spec-lock approval 还同批 emit `event:phase_advanced`(actor `human:` only) |
...
| `loaf doctor --migrate-v2` <!-- inventory:future reason="doctor currently ships --rebuild only; migrate-v2 is not a live Commander option" --> | `migration:snapshot_imported`(actor `migration:*`;journal seq=0/1 only) |
...
- **`--format json` 是 stable contract**(跟 runtime schema owners 的字段集 lock-step 演进;schema_version bump 时同步)
...
|---|---|---|
| `loaf start` | `start: '<feature>' created → TRIAGE.score` | `next: <runnable buildNextOutput non-blocking command + original feature selector>` |
| `loaf advance` | `advance: <prev sub-state> → <new sub-state>` | 来自 post-mutation snapshot 的 canonical next action |
| `loaf spec submit` | `spec submit: spec_version=N, locked=false` | 来自 post-mutation snapshot 的 canonical next action |
...
| `loaf tasks submit` | `tasks submit: N tasks` | blocking action pointer: `next: loaf next <original feature selector> --format json`; `buildNextOutput` remains routing authority |
...
| `loaf gate decide spec-lock` (approve) | `gate decide: spec-lock approved by <actor>` | 来自 post-batch `EXECUTE.plan` snapshot 的 canonical next action |
| `loaf gate decide verify-accept` (approve) | `gate decide: verify-accept approved by <actor>` | 来自 post-gate `VERIFY.accept` snapshot 的 settle/deliver canonical branch |
| `loaf gate decide <G>` (reject) | `gate decide: <gate> rejected by <actor>` | — |
| `loaf settle` | `settle: <from> → SETTLE.lessons` | 与随后 `loaf next` 相同的 scoped canonical command |
...
| **tail-corruption**(rev 5.0,ADR-0005 §4.13 + Gate #4)| `.loaf/<feature>/journal.jsonl` 末尾 | 末行 partial JSON / 末 batch `batch_index < batch_count - 1` 或 batch 末 entry partial / sha256(last entry) ≠ `_meta.last_entry_line_hash` | `--fix`:所有 truncate 只使用扫描时记录的原始行 pre-offset；单 entry partial → truncate 末行；batch incomplete → truncate 整批至 batch 第一 entry 之前；rewrite `snapshots/_meta.json`；若 truncate 跨过 `last_applied_seq` → 同时跑 `--rebuild`。若 tail 是 valid envelope 但 kind 未知或 `entry_schema_version` 高于本 binary 对该 kind 的支持版本，禁止 truncate 并报 `JOURNAL_TAIL_REQUIRES_NEWER_LOAF`，要求先升级。|
| **stale-tmp**(rev 5.0)| `.loaf/<feature>/{journal.jsonl,snapshots/*,attachments/**}.tmp-*` | 任意 `.tmp-*` 残留 + mtime > 60s + 无 active lock | `--fix`:`unlink` 全部 |
| **snapshot-seq-mismatch**(rev 5.0,Gate #5)| `.loaf/<feature>/snapshots/_meta.json` | `last_applied_seq` < 实际 journal 末 entry seq,或 `last_entry_line_hash` mismatch | 不自动 fix;打印 `SNAPSHOT_STALE_REBUILD_REQUIRED` 提示跑 `loaf doctor --rebuild`(reader contract 永不静默 fallback,ADR-0005 §3.6)。**Phase 15 SC3 + ticket #12**:`projection-loader`(`status` / `tasks list` / `pending list` / `finding list` / `evidence list` 五个只读命令)在消费 `snapshots/<kind>.json` 前必跑 Gate #5 fast-check + 第二次 cached-M0 post-read fast-check(M0-anchored linearization guard);任一 fail → exit 2 `SNAPSHOT_STALE_REBUILD_REQUIRED`,9-reason taxonomy(`detail.reason`)包括 reader 原 5 reason(`journal_missing` / `journal_empty` / `tail_offset_mismatch` / `tail_hash_mismatch` / `trailing_partial_line`)+ loader 加 4 reason(`meta_missing` / `meta_invalid {cause: json_parse \| schema}` / `projection_missing {projection_kind}` / `projection_invalid {projection_kind, cause}`)。`tasks.json` 缺失若 `state.based_on.tasks === 0` 走 valid-empty(writer skip,§14)而非 `projection_missing`;pre-`loaf start` dir(journal + meta 均缺,或 `isEmptyMeta` sentinel + journal 空)分类成 `NO_SESSION` 而非 stale。其他需要 canonical entries 的只读命令(如 `journal list`)仍走 `loadSession` full replay |
...
loaf doctor --format json       # 结构化输出,CI / TUI 消费
```

`tailRecovery` 的 version-aware 判定只信任 envelope 的 `seq`、`entry_id`、`at`、`actor`、`entry_schema_version`、`kind`、`payload` 存在性以及可选 batch 三元组结构；不解释未知 kind 的 payload，也容忍额外 envelope 字段。满足该稳定 envelope 后，未知 kind 或高于 `ENTRY_SCHEMA_VERSIONS[kind]` 的版本即视为 newer writer 证据，且在打开文件进行 truncate 之前 fail closed。`doctor --check-tail` 仍未接线。
...
机器表达:`src/cli/input-schemas.ts::INPUT_SCHEMAS` 5 个 schema 全部为 `z.union([T, z.array(T).nonempty()])` 形态。

**违反方式与诊断**:
- 外部进程直写检测(**v0.1.0 未实装,延后后续 SC**):设计意图为 `loaf hook closure-check`(`Stop` 事件)对比 artifact mtime 与 last `loaf <cmd>` exit time,异常 mtime 告警(不阻塞,记 trace.jsonl)。**当前实装** `closure-check` 仅跑 projection-freshness + orphan-evidence covers[] + open-findings 摘要三项 read-only 检查(见 §11 表下 v0.1.0 note),不做 mtime 比对
- Lock 抢占:并发两个 `loaf <cmd>` → 后者 bounded wait / 超时 → 报 `LOCK_TIMEOUT`;holder 元数据保留在结构化 detail。
- Stale lock:后续 writer 仅在 PID 可证已退出、且二次读取确认 owner token 与文件 generation 未变化时回收。malformed/incomplete lease 报 `LOCK_INVALID` 并 fail closed；当前没有自动清理命令。

机器表达见 `src/core/concurrency-contract.ts::CONCURRENCY_INVARIANTS`。
...
**rev 5.1:quick / light / standard + lessons 说明**:quick / light / standard 三档均跳过 SETTLE.lessons sub_state,但 `loaf lessons add` 本身在任意非 DONE sub_state 可调(append-only,不动 state machine)。命令 emit `lesson:recorded`,由 projection writer 重建顶层 `.loaf/<feature>/lessons.md`。兼容桥只在 `selectLessonEntries()`：新 kind + legacy `evidence:added` lesson heuristic 永久双读并保持 journal 顺序。legacy lesson 永远保留 evidence projection/status/board/TUI/resume-pack 可见性;新 lesson 永远不进入这些 evidence-derived surface,也不满足 coverage。loaf-skill 可以 stderr **soft suggestion** "已记 lesson;考虑下个相似 feature 用 deep profile 进 SETTLE.lessons 强化复利",但**不**强制 protocol 升 profile。
...
- `resume-pack.json.recent_lessons`(无消费者,不扩展 handoff surface)

v1 的核心目标是 protocol 可靠,不是知识复利自动化。跨 feature lesson 查阅仍统一 grep `.loaf/*/lessons.md`;`loaf evidence list` 已实现，但新 `lesson:recorded` 永远不进入 evidence projection，因此该命令不会列出新 lesson。
...
> - **`feature: string`**(C9')—— **Phase 16 SC-7**:canonical source is `SessionStartedPayload.feature` from the journal (NOT `path.basename(featureDir)` — tmpdir paths in tests break that derivation). Kebab-case is convention for production but NOT enforced at journal level (`SessionStartedPayload.feature` is `.min(1)`); the registry projection accepts whatever the journal carries. RegistryFile path 不含 feature 上下文,TUI 启动单文件读需要。**StateJson 不 carry 同字段**(其 path 已 carry feature dir,reader 一行 path.basename derive)
...
任何 hook / sub-agent / CLI 在调用 `AskUserQuestion` 之前,通过 mutator append 到队列尾(CLI 分配 PEND-id,在 per-session lock 内,见 §11.2 + `src/core/concurrency-contract.ts`):
...
**v1.0.0 tag 的硬判据**(对应 `src/core/version-contract.ts::V1_DONE_CRITERIA`):
...
4. 三件套文档(protocol.md / machine-contract.md / index.html)与实际 CLI 行为对齐
...
loaf-cli v1 是 **legacy Python 原型** 的 successor,from scratch —— 实现不复用一行旧代码。原型在 early-draft 内部曾被称为 "v2"(rev 3.2 之前文档残留 "v3 是 v2 的 successor" 说法,与当时 runtime 的 `SCHEMA_VERSION=1` + §15-16 直接矛盾,已在 rev 3.2 修正)。
...
```
runtime schema owners 里的 enum / ID / 字段名 → 稳定英文 ID,永不本地化
i18n/<lang>.json bundle           → 人类可读 label + diagnostic 模板
TUI / CLI 人类输出                → 通过 bundle 渲染
JSON 输出(`--format json`)       → 永远是 stable ID,不走 i18n
```
...
**职责边界明示**(rev 4.3,ADR-0004 A2):**shape enforcement = CLI 责任,workflow content = skill 责任**。Tier 1 mutator 的所有 shape 校验(id 分配 / 三选一可验证 / 6 evidence kind 字段矩阵 / 6×6 finding 合法性 / attachment hash+mime+canonicalization)集中在 loaf-cli;若放 skill,每个 3rd-party workflow skill(Wang / GSD / openspec / 内部 ad-hoc)都要重复实现 runtime schema 子集,且 skill 改文件与 `loaf check` 之间引入 race window。CLI 做一次,所有 skill 共享 — 这是 ADR-0001「shape 在协议,content 在 skill」原则的直接 corollary。

### 19.2 集成契约 = CLI --help + schema --format=json

外部 skill 集成 loaf-cli 的方式:

```bash
# 1. 读 schema(自描述)
loaf evidence schema --format=json | jq        # 拿到 JSON Schema
